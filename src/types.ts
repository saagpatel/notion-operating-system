...
import type { WriteVerification } from "./publishing/write-verification.js";
...
export const SafeDefaultsSchema = z
	.object({
		...
		// Per-destination write-verification knob (PR-3). Left optional (no
		// schema-level default) rather than defaulted so a missing key does not
		// force every hand-built SafeDefaults literal in tests/config to name it;
		// consumers resolve a missing value to "warn" at read time.
		verifyWrites: z.enum(["warn", "fail", "off"]).optional(),
	})
	.default({
...
export type DestinationRegistryConfig = z.infer<
	typeof DestinationRegistrySchema
>;
...
export type ResolvedDestination =
	| ResolvedPageDestination
	| ResolvedDataSourceDestination;
...
export interface PublishSummary {
	...
	// Present on live runs that perform a readback; absent on dry runs, which
	// stay read-only and gain no verification (PR-3).
