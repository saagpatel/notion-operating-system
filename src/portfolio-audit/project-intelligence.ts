...
const PROJECTS_ROOT =
	process.env["PROJECTS_ROOT"] ?? path.join(homedir(), "Projects");
...
export async function buildProjectIntelligenceDataset(): Promise<{
	...
	const inventoryByKey = new Map(
		inventory.map((item) => [normalizeKey(item.projectName), item] as const),
	);
	const detailByPath = sources.detailEntriesByPath;

	const projects = await Promise.all(
		inventory.map(async (project) => {
			const workbookRow = sources.workbookRows.get(
				normalizeKey(project.projectName),
			);
			const detail =
...
export function buildProjectProfileMarkdown(
	...
	const strategicValue =
		...
		project.monetizationValue !== project.valueOutcome
			? project.monetizationValue
			: "";
	...
	const lines = [
		...
		"",
		project.primaryUser || project.valueOutcome ? "## Product / User Fit" : "",
		...
		"",
		"## Current Reality",
		project.lastMeaningfulWork
			? `- Last meaningful work: ${project.lastMeaningfulWork}`
			: "",
		project.whatWorks ? `- What works: ${project.whatWorks}` : "",
		project.missingCorePieces
			? `- Missing core pieces: ${project.missingCorePieces}`
			: "",
		...
		project.projectShape.length > 0
			? `- Project shape: ${project.projectShape.join(", ")}`
			: "",
		project.deploymentSurface.length > 0
			? `- Deployment surface: ${project.deploymentSurface.join(", ")}`
			: "",
		project.keyIntegrations
			? `- Key integrations: ${project.keyIntegrations}`
			: "",
		`- Docs quality: ${project.docsQuality || "Unknown"}`,
		`- Test posture: ${project.testPosture || "Unknown"}`,
		project.contextDocs.length > 0
			? `- Context docs: ${project.contextDocs.join(", ")}`
			: "",
		"",
		"## Shipping Path",
		project.nextMove ? `- Next move: ${project.nextMove}` : "",
		project.biggestBlocker
			? `- Biggest blocker: ${project.biggestBlocker}`
			: "",
		project.effortToDemo ? `- Effort to demo: ${project.effortToDemo}` : "",
		project.effortToShip ? `- Effort to ship: ${project.effortToShip}` : "",
		"",
		"## Risks / Unknowns",
		project.knownRisks ||
			project.projectHealthNotes ||
			"No material risk summary captured yet.",
		"",
		"## Linked Activity",
		...
		project.projectHealthNotes
			? `- Health notes: ${project.projectHealthNotes}`
			: "",
		project.canonicalNotes
			? `- Legacy audit note: ${project.canonicalNotes}`
			: "",
	].filter(Boolean);
...
export function deriveCurrentState(input: {
	...
	if (
		readiness.includes("ship-ready") ||
		readiness.includes("complete") ||
		(completion ?? 0) >= 90
	) {
...
export function derivePrimaryRunCommand(input: {
	...
	const preferredScript =
		["dev:tauri", "tauri:dev", "dev", "start", "desktop", "app", "web"].find(
			(script) => packageScripts.includes(script),
		) ?? packageScripts[0];
...
export function deriveDocsQuality(input: {
	...
	if (
		(input.hasClaude && input.contextDocsCount >= 2) ||
		(input.hasAgents && input.hasClaude)
	) {
		return "Strong";
	}
	if (
...
export function deriveTestPosture(input: {
	...
	const text =
		`${input.codeQuality ?? ""} ${input.completion ?? ""}`.toLowerCase();
	if (
		/strong|comprehensive|158|130\+|85\+|44 test|328 tests|33\/33|18\/18|37 tests/i.test(
			text,
		)
	) {
		return "Strong";
	...
	if (
		text.includes("tests: not yet") ||
		text.includes("tests: low") ||
		text.includes("tests: none")
	) {
		return "Sparse";
	}
	return "Unknown";
...
function buildProjectRow(input: {
	...
	const canonicalPipelineStage =
		firstNonEmpty(
			workbookRow?.["Pipeline Stage"],
			deriveFallbackPipelineStage(canonicalStatus),
		) ?? "";
	...
	const completion =
		firstNonEmpty(
			detail?.completion,
			extractLegacyField(canonicalNotes, "Legacy completion"),
		) ?? "";
	const readiness =
		firstNonEmpty(
			detail?.readiness,
			extractLegacyField(canonicalNotes, "Legacy readiness"),
		) ?? "";
	const canonicalSummary =
		firstNonEmpty(
			detail?.summary,
			extractSummaryFromNotes(canonicalNotes),
			signals.claudeOverview,
		) ?? "";
	const stack =
		firstNonEmpty(
			detail?.stack,
	...
	const lastActive =
		firstNonEmpty(
			signals.gitLastActive,
			signals.filesystemLastActive,
			workbookRow?.["Date Updated"],
		) ?? "";
	...
	const buildMaturity = deriveBuildMaturity(
		completion,
		readiness,
		currentState,
	);
	const shipReadiness = deriveShipReadiness(
		readiness,
		currentState,
		completion,
	);
	...
	const evidenceConfidence = deriveEvidenceConfidence({
		...
		testPosture,
		hasManifest:
			signals.hasPackageJson || signals.hasCargoToml || signals.hasPyproject,
	});
	...
	const nextMove =
		firstNonEmpty(
			...
			detail?.quickWins,
			detail?.toShipReady,
			deriveFallbackNextMove(primaryRunCommand),
		) ?? "";
	const oneLinePitch =
		canonicalSummary ||
		(firstNonEmpty(detail?.problemSolved, detail?.strategicValue) ?? "");
	const momentum = deriveMomentum(lastActive);
	const deploymentSurface = deriveDeploymentSurface(
		stack,
		canonicalSummary,
		input.inventory.sourceGroup,
	);
	const projectShape = deriveProjectShape(
		canonicalCategory,
		canonicalSummary,
		input.inventory.sourceGroup,
	);
	const setupFriction = deriveSetupFriction(detail?.operational, signals);
	const runsLocally = deriveRunsLocally(
		detail?.operational,
		currentState,
		signals,
	);
	const effortToDemo = deriveEffortToDemo(
		detail?.toMvp,
		buildMaturity,
		setupFriction,
	);
	const effortToShip = deriveEffortToShip(detail?.toShipReady, shipReadiness);
	const portfolioCall = derivePortfolioCall(
		canonicalVerdict,
		currentState,
		buildMaturity,
	);
	const valueOutcome =
		firstNonEmpty(
			detail?.strategicValue,
			deriveValueFromCategory(canonicalCategory, canonicalSummary),
		) ?? "";
	...
	const projectHealthNotes = compactText([
		...
		detail?.codeQuality,
		detail?.operational,
		signals.claudeCurrentPhase
			? `Backfilled from CLAUDE.md. ${signals.claudeCurrentPhase}`
			: "",
	]);
	const needsReview =
		...
		input.inventory.relativePath ===
			"claude-code/production/rag-knowledge-base";
	...
	return {
		...
		lastMeaningfulWork:
			firstNonEmpty(
				detail?.whatWorks,
				deriveLastMeaningfulWork(lastActive, currentState),
			) ?? "",
		primaryUser:
			firstNonEmpty(
				detail?.intendedUser,
				deriveDefaultUser(canonicalCategory, canonicalSummary),
			) ?? "",
		...
		monetizationValue:
			firstNonEmpty(
				detail?.strategicValue,
				deriveStrategicValue(canonicalCategory, canonicalSummary),
			) ?? "",
		keyIntegrations,
		projectHealthNotes,
		knownRisks,
...
async function loadWorkbookRows(): Promise<Map<string, WorkbookRow>> {
	...
	for (let rowIndex = 2; rowIndex <= worksheet.rowCount; rowIndex += 1) {
		...
		const projectName = String(
			row.getCell(headerIndex.get("Project Name") ?? 1).value ?? "",
		).trim();
		if (!projectName) {
			continue;
		}
		const record = Object.fromEntries(
			CANONICAL_COLUMNS.map((column) => [
				column,
				String(row.getCell(headerIndex.get(column) ?? 1).value ?? "").trim(),
			]),
		) as WorkbookRow;
...
async function discoverInventoryProjects(): Promise<InventoryProject[]> {
	...
	for (const entry of entries) {
		...
		if (
			...
			EXCLUDED_ROOT_NAMES.has(entry.name) ||
			ROOT_GROUPS.has(entry.name)
		) {
			continue;
		}
		discovered.push({
	...
	for (const groupName of GROUP_DIRECTORIES.filter(
		(group) => group !== "GrokPRJs",
	)) {
		...
		for (const entry of groupEntries) {
			...
			discovered.push({
				...
				relativePath,
				sourceGroup:
					groupName === "FunGamePrjs"
						? "FunGamePrjs - Build-Ready Staging"
						: groupName,
	...
	const ragKbPath = path.posix.join(
		"claude-code",
		"production",
		"rag-knowledge-base",
	);
	...
	for (const entry of entries) {
		...
		if (entry.name === "claude-code") {
			...
			if (ragKbStat?.isDirectory()) {
				discovered.push({
					projectName:
						PATH_NAME_OVERRIDES.get(ragKbPath) ?? "RAG Knowledge Base",
					relativePath: ragKbPath,
		...
		if (ROOT_GROUPS.has(entry.name)) {
			const groupEntries = await safeReadDir(
			...
			for (const child of groupEntries) {
				...
				discovered.push({
					projectName:
						PATH_NAME_OVERRIDES.get(relativePath) ?? child.name.trim(),
					relativePath,
...
function buildScopeReconciliation(input: {
	...
	const auditedKeys = new Set(
		input.auditedProjects.map((project) => normalizeKey(project.projectName)),
	);
	...
	for (const project of input.discoveredAll) {
		...
		if (project.sourceBucket === "GrokPRJs") {
			...
			notes =
				"Legacy Grok-built project kept out of the current audited portfolio.";
		} else if (
			project.relativePath.includes("ready") ||
			project.relativePath.includes("readiness")
		) {
			classification = "Staging duplicate";
			notes =
				"Readiness or staging artifact better tracked under the base project.";
		}
	...
	const unmatchedAudited = input.auditedProjects
		.filter(
			(project) => !input.inventoryByKey.has(normalizeKey(project.projectName)),
		)
...
function parseDetailBlock(
	...
	return {
		...
		stack: firstNonEmpty(
			extractListItem(technicalBlock, "Stack"),
			extractInlineField(text, "Stack"),
			extractInlineField(text, "Technical Profile"),
		),
		architecture: firstNonEmpty(
			extractListItem(technicalBlock, "Architecture"),
			extractInlineField(text, "Architecture"),
		),
		...
		aiUsage: firstNonEmpty(
			extractListItem(technicalBlock, "AI/LLM usage"),
			extractInlineField(text, "AI/LLM"),
		),
		storage: firstNonEmpty(
			extractListItem(technicalBlock, "Data storage"),
			extractInlineField(text, "Storage"),
		),
		entryPoint: firstNonEmpty(
			extractListItem(technicalBlock, "Entry point"),
			extractInlineField(text, "Entry"),
		),
		completion: extractInlineField(text, "Completion"),
		readiness: extractInlineField(text, "Readiness"),
		whatWorks: firstNonEmpty(
			extractInlineField(text, "What works"),
			extractInlineField(text, "Works"),
		),
		...
		verdict: firstNonEmpty(
			extractBulletField(strategicBlock, "Verdict"),
			extractInlineField(text, "Verdict"),
		),
		priority: firstNonEmpty(
			extractBulletTailField(strategicBlock, "Priority"),
			extractInlineField(text, "Priority"),
		),
		blocker: firstNonEmpty(
			extractBulletTailField(strategicBlock, "Blocker"),
			extractInlineField(text, "Blocker"),
		),
		quickWins: firstNonEmpty(
			extractBulletTailField(strategicBlock, "Quick wins"),
			extractInlineField(text, "Quick wins"),
		),
		integration: firstNonEmpty(
			extractBulletTailField(strategicBlock, "Integration"),
			extractInlineField(text, "Integration"),
		),
	};
}
...
async function inspectProjectSignals(
	projectPath: string,
	detail?: DetailEntry,
): Promise<ProjectSignals> {
	const rootFiles = await safeReadDir(projectPath);
	const filenames = new Set(
		rootFiles.filter((entry) => entry.isFile()).map((entry) => entry.name),
	);
	...
	const hasCargoToml =
		...
		Boolean(await safeStat(path.join(projectPath, "src-tauri", "Cargo.toml")));
	const hasPyproject =
		filenames.has("pyproject.toml") || filenames.has("requirements.txt");
	...
	const contextDocs = [
		...
		.filter(
			(name) =>
				filenames.has(name) ||
				Boolean(statExists(path.join(projectPath, name))),
		)
		.map((name) => name);

	const packageJson = hasPackageJson
		? await safeReadJson(path.join(projectPath, "package.json"))
		: null;
	...
	const claudeContext = hasClaude
		? await readClaudeContext(path.join(projectPath, "CLAUDE.md"))
		: emptyClaudeContext();

	const primaryContextDoc =
		firstNonEmpty(
...
async function readClaudeContext(
	filePath: string,
): Promise<{ overview: string; currentPhase: string; techStack: string }> {
	try {
		const text = await fs.readFile(filePath, "utf8");
		return {
			overview: collapseMarkdownText(extractMarkdownSection(text, "Overview")),
			currentPhase: collapseMarkdownText(
				extractMarkdownSection(text, "Current Phase"),
			),
...
function deriveFallbackStatus(signals: ProjectSignals): string | undefined {
	...
	if (
		/feature complete|v1\.0|phase complete|all 4 phases implemented/.test(phase)
	) {
		return "Shipped";
	}
	if (
...
function deriveFallbackCategory(input: {
	summary: string;
	projectName: string;
}): string | undefined {
	const text = `${input.projectName} ${input.summary}`.toLowerCase();
	if (
		/argument|reasoning|decision|llm|chain-of-thought|research synthesis/.test(
			text,
		)
	) {
	...
	if (
		/log stream|ambient monitoring|claude code build logs|menu bar/.test(text)
	) {
		return "Dev Tool";
	}
	...
	if (
		/desktop|macos|local-first|overlay|notifications|warranty|return window|deep-work/.test(
			text,
		)
	) {
		return "Desktop App";
...
function deriveFallbackVerdict(
	status: string,
	summary: string,
): string | undefined {
	if (status === "Shipped") {
...
function deriveDeploymentSurface(
	...
	if (
		values.size === 0 &&
		/commercial saas|portal|statuspage|resume|document|job/.test(text)
	) {
		values.add("Web");
	}
...
function deriveProjectShape(
	...
	if (
		/game|roguelike|companion|ecosystem|crawler/.test(text) ||
		sourceGroup.includes("Game")
	) {
		values.add("Game");
	}
...
function deriveEffortToDemo(
	toMvp: string | undefined,
	buildMaturity: string,
	setupFriction: string,
): string {
	const text = (toMvp ?? "").toLowerCase();
	if (
		/<\s*2|30 min|15 min|20 min|already/.test(text) ||
...
function deriveEffortToShip(
	toShip: string | undefined,
	shipReadiness: string,
): string {
	const text = (toShip ?? "").toLowerCase();
	if (
		/already shipped|already at mvp|already/.test(text) ||
		shipReadiness === "Ship-Ready"
	) {
		return "<1 day";
	}
	if (
		/2-3 days|30-60 min|one-time/.test(text) ||
...
function buildStartHere(
	...
	const docPart = primaryContextDoc
		? `Open \`${primaryContextDoc}\``
		: "Open the strongest available repo doc";
	const commandPart = primaryRunCommand
		? `then run \`${primaryRunCommand}\``
		: "then confirm the right local boot command";
	const actionPart = firstNonEmpty(quickWins, toMvp)
		? `, then tackle: ${firstNonEmpty(quickWins, toMvp)}`
		: "";
...
function deriveBlockerFromOperational(
	operational?: string,
): string | undefined {
	if (!operational) {
		return undefined;
...
function deriveSetupPrerequisites(input: {
	...
	const values = new Set<string>();
	const text =
		`${input.detail?.stack ?? ""} ${input.detail?.integrations?.join(", ") ?? ""} ${input.detail?.operational ?? ""}`.toLowerCase();
	if (input.hasPackageJson) {
		values.add("Node.js");
	}
	if (input.hasCargoToml) {
...
function detectPackageManager(
	...
	if (
		statExists(path.join(projectPath, "bun.lockb")) ||
		statExists(path.join(projectPath, "bun.lock"))
	) {
		return "bun";
	}
	return "npm";
...
function extractBulletTailField(
	text: string,
	label: string,
): string | undefined {
...
function extractLegacyField(notes: string, label: string): string {
	const match = notes.match(
		new RegExp(`${escapeRegExp(label)}:\\s*(.*?)(?=\\.\\s+[A-Z]|$)`),
	);
	return match?.[1]?.trim() ?? "";
}
...
function formatLinkedItems<T extends { title: string; url: string }>(
	items: T[],
	label: (item: T) => string,
): string {
	if (items.length === 0) {
