...
export async function runSupportDatabaseHygienePass(
  ...
  const approvalRequiredReadback = [
    "archive_ids_absent",
  ...
  const hasEffects = approvalPlan.effect_count > 0;
  const approvalEnvelope = hasEffects
    ? {
        ...
        artifact_digest: planDigest(approvalPlan),
        bounds: {
          allowed_effect_count: approvalPlan.effect_count,
          max_deletions: approvalPlan.archive_page_ids.length,
        },
        required_readback: approvalRequiredReadback,
      }
    : null;
  let envelope: IrreversibleActionEnvelopeV1 | undefined;
  const executeLive = flags.live && hasEffects;
  if (executeLive) {
    if (!flags.approval) {
      throw new AppError(
    ...
    validateEnvelope({
      ...
      requiredReadback: approvalRequiredReadback,
    });
