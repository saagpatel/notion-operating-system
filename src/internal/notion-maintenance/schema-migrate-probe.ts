...
export async function runProbe(input: {
  ...
  if (!input.live) {
    const approvalEnvelope = input.actionId
      ? (() => {
          assertPathSafeActionId(input.actionId!);
          const plan = probePlan(input.dataSourceId, input.actionId!);
          return {
...
async function main(): Promise<void> {
    ...
    process.stdout.write(
      renderInternalScriptHelp({
        ...
        options: [
          ...
          {
            flag: "--action-id <id>",
            description: "Path-safe action id used to render the exact dry-run approval plan.",
          },
        ],
        notes: [
          "Default execution is side-effect-free and does not construct a Notion client.",
          "Use --action-id during dry-run before creating a live approval envelope.",
        ],
      }),
    );
    return;
  ...
  const actionIdIndex = argv.indexOf("--action-id");
  const actionId = actionIdIndex >= 0 ? argv[actionIdIndex + 1] : undefined;
  const result = await runProbe({
    live,
    dataSourceId,
    actionId: live ? undefined : actionId,
