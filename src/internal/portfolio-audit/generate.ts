...
function parseReport(reportText: string): ParsedReport {
	...
	const summaryIndex = generatedSummaryMatch
		? reportText.indexOf(generatedSummaryMatch[0])
		: -1;

	if (summaryIndex === -1 || inventoryStart === -1) {
		throw new Error(
			"Could not find expected report sections in PORTFOLIO-AUDIT-REPORT.md",
		);
	}

	const inventorySections = reportText
		.slice(inventoryStart, summaryIndex)
		.trim();
	const summaryEntries = parseSummaryTable(reportText.slice(summaryIndex));
	const { detailEntries, detailEntriesByPath } =
		parseDetailSections(inventorySections);
...
function parseDetailBlock(
	...
	return {
		...
		stack:
			extractInlineField(text, "Stack") ?? extractTechnicalProfileStack(text),
		externalIntegrations: extractTechnicalProfileListItem(
			text,
...
async function buildDataset(
	...
	const rows = await Promise.all(
		discoveredProjects.map(async (inventory) => {
			const detail =
				parsedReport.detailEntriesByPath.get(
					normalizePathKey(inventory.relativePath),
				) ??
			...
			const summary =
				...
				parsedReport.summaryEntries.get(
					normalizeKey(detail?.headingName ?? ""),
				);
			const registry =
			...
			if (pathBasename !== pathBasename.trim()) {
				anomalies.push(
					"Filesystem folder name contains leading or trailing whitespace.",
				);
			}
			...
			return buildRow({
				...
				summary,
				registry,
				inspectedStack: await inspectProjectStack(
					path.join(PROJECTS_ROOT, inventory.relativePath),
				),
				anomalies,
			});
		}),
	...
	for (const row of rows) {
		...
		if (!row.Notes.includes("Staging copy")) {
			row.Notes = appendSentence(
				row.Notes,
				"Staging copy kept as a separate local project row.",
			);
		}
	}
...
function buildRow(context: ProjectContext): ProjectDatasetRow {
	const legacyCategory = firstNonEmpty(
		context.summary?.legacyCategory,
		context.detail?.legacyCategory,
	);
	const completion = firstNonEmpty(
		context.summary?.completion,
		context.detail?.completion,
	);
	const readiness = firstNonEmpty(
		context.summary?.readiness,
		context.detail?.readiness,
	);
	const legacyVerdict = firstNonEmpty(
		context.summary?.legacyVerdict,
		context.detail?.legacyVerdict,
	);
	const priority = firstNonEmpty(
		context.summary?.priority,
		context.detail?.priority,
	);
	const stack = firstNonEmpty(context.detail?.stack, context.inspectedStack);
	const keyIntegrations =
		context.detail?.externalIntegrations?.join(", ") ?? "";
	...
	const row: ProjectDatasetRow = {
		...
		"Date Scored": "",
		"Date Updated": safeDateFromPath(
			path.join(PROJECTS_ROOT, context.inventory.relativePath),
		),
	...
	if (row.Status === "Merged" && !row["Merged Into"]) {
		row.Notes = appendSentence(
			row.Notes,
			"Merged target needs manual confirmation.",
		);
	}
	if (!row.Category) {
		row.Notes = appendSentence(
			row.Notes,
			"Category left blank under the conservative Notion mapping.",
		);
	}
	...
	if (
		context.inventory.projectName === "PomGambler" ||
		context.inventory.projectName === "PomGambler-prod"
	) {
		row.Category = row.Category || "Desktop App";
	}
	if (
		context.inventory.relativePath ===
...
function deriveStatus(
	...
	if (
		summaryText.includes("v1 shipped") ||
		summaryText.includes("already shipped") ||
	...
	if (
		...
		(typeof completionValue === "number" &&
			completionValue >= 85 &&
			(verdict.includes("ship") || verdict.includes("release")))
	) {
		return "Handoff Ready";
	}
	if (
		(verdict.includes("archive") || verdict.includes("delete")) &&
		!verdict.includes("ship") &&
		!verdict.includes("release")
	) {
		return "Abandoned";
	}
...
function derivePipelineStage(
	...
	if (
		(registry === "active" || registry === "recent") &&
		!verdict.includes("archive")
	) {
...
function deriveCategory(
	...
	const source =
		`${legacyCategory ?? ""} ${stack ?? ""} ${summary ?? ""}`.toLowerCase();
	...
	if (
		source.includes("dev tool") ||
		source.includes("api client") ||
		source.includes("postgresql ide")
	) {
		return "Dev Tool";
	}
	...
	if (
		source.includes("file transfer") ||
		source.includes("encryption") ||
		source.includes("topology")
	) {
		return "IT Tool";
	}
	if (
		...
		source.includes("tutorial generation")
	) {
...
function extractMergedInto(legacyVerdict?: string): string {
	const match = (legacyVerdict ?? "").match(
		/merge(?:d)?(?: into)?\s+([A-Za-z0-9().\- ]+)/i,
	);
	return match?.[1]?.trim() ?? "";
}
...
function buildNotes(context: ProjectContext): string {
	...
	if (
		...
		const registryBits = [
			...
			context.registry?.contextQuality
				? `context ${context.registry.contextQuality}`
				: "",
		].filter(Boolean);
...
async function inspectProjectStack(projectPath: string): Promise<string> {
	...
	if (packageJson) {
		...
		if (
			deps.typescript ||
			(await safeStat(path.join(projectPath, "tsconfig.json")))
		) {
...
function buildMarkdownReport(
	...
	return [
		...
		"## 1. Audit Methodology",
		"",
		"- Scope: local-only portfolio audit using the current `~/Projects` filesystem, the existing detailed audit narrative, and `project-registry.md` as supporting context.",
		...
		buildCountTable(
			"Pipeline Stage",
			"Projects",
			countBy(dataset, "Pipeline Stage"),
		),
		"",
		"### Category",
		"",
...
function buildCountTable(
	...
	const rows = entries
		...
		.map(
			([label, value]) => `| ${escapeCell(label || "(blank)")} | ${value} |`,
		);
	return [
...
function buildReviewTable(rows: ProjectDatasetRow[]): string {
	...
	return [
		...
		...rows.map(
			(row) =>
				`| ${escapeCell(row["Project Name"])} | ${escapeCell(row.Notes)} |`,
		),
	].join("\n");
}
...
function buildFieldCoverage(
	...
	return Object.fromEntries(
		CANONICAL_COLUMNS.map((column) => [
			column,
			dataset.filter((row) => row[column].trim().length > 0).length,
		]),
...
async function writeWorkbook(dataset: ProjectDatasetRow[]): Promise<void> {
	...
	for (const row of dataset) {
		projectsSheet.addRow(
			Object.fromEntries(
				CANONICAL_COLUMNS.map((column) => [column, row[column]]),
			),
		);
	}
	...
	const sections: Array<{ title: string; rows: Array<[string, number]> }> = [
		{ title: "Status counts", rows: countBy(dataset, "Status") },
		{
			...
			rows: countBy(dataset, "Pipeline Stage"),
		},
	...
	for (const section of sections) {
		...
		rowIndex += 1;
		for (const [label, count] of section.rows.filter(
			([, value]) => value > 0,
		)) {
	...
	analysisSheet.getCell("D4").value =
		"Projects missing one or more high-value fields";
...
function extractTechnicalProfileListItem(
	text: string,
	label: string,
): string[] | undefined {
	const block = text.match(
