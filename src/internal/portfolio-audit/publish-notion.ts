...
async function main(): Promise<void> {
	...
	const parentPageId = normalizeNotionId(
		extractNotionIdFromUrl(parent) ?? parent,
	);
	const databaseTitle = flags.title ?? DEFAULT_DATABASE_TITLE;

	const [inventory, workbookRows] = await Promise.all([
		discoverProjects(),
		loadWorkbookRows(),
	]);
	...
	const samplePage = flags.updateExisting
		...
		: await populateIntoEmptyDatabase(
				sdk,
				markdownApi,
				created.dataSourceId,
				importRows,
...
async function loadWorkbookRows(): Promise<CanonicalWorkbookRow[]> {
	...
	const headerRow = headerValues.map((value: ExcelJS.CellValue) =>
		String(value ?? ""),
	);
	...
	const missingHeaders = CANONICAL_COLUMN_NAMES.filter(
		(columnName) => !headerIndexes.has(columnName),
	);
	if (missingHeaders.length > 0) {
		throw new AppError(
			`Workbook is missing expected columns: ${missingHeaders.join(", ")}`,
		);
	}
	...
	for (let rowIndex = 2; rowIndex <= worksheet.rowCount; rowIndex += 1) {
		...
		const projectName = String(
			row.getCell(headerIndexes.get("Project Name") ?? 1).value ?? "",
		).trim();
		if (!projectName) {
			continue;
		}
		...
		const record = Object.fromEntries(
			CANONICAL_COLUMN_NAMES.map((columnName) => [
				columnName,
				String(
					row.getCell(headerIndexes.get(columnName) ?? 1).value ?? "",
				).trim(),
			]),
...
function buildImportRows(
	workbookRows: CanonicalWorkbookRow[],
	inventory: InventoryProject[],
): ProjectImportRow[] {
	const inventoryByProjectName = new Map(
		inventory.map((project) => [project.projectName, project] as const),
	);
	...
	return workbookRows.map((row) => {
		...
		const integrationTags = deriveIntegrationTags(
...
function parseNotes(notes: string): {
	...
	const markers = [
		notes.indexOf(" Legacy completion:"),
		notes.indexOf(" Registry:"),
	].filter((value) => value >= 0);
	const firstMarkerIndex = markers.length > 0 ? Math.min(...markers) : -1;
	const summary = (
		firstMarkerIndex >= 0 ? notes.slice(0, firstMarkerIndex) : notes
	)
		.trim()
		.replace(/\s+\.$/, ".");

	const completionMatch = notes.match(
		/Legacy completion:\s*(.*?)(?=\.\s+Legacy readiness:|\.\s+Registry:|$)/,
	);
	const readinessMatch = notes.match(
		/Legacy readiness:\s*(.*?)(?=\.\s+Registry:|$)/,
	);
	const registryMatch = notes.match(
		/Registry:\s*registry\s+([^,]+),\s*tool\s+([^,]+),\s*context\s+([^.]+)\.?/i,
	);

	let remaining =
		firstMarkerIndex >= 0 ? notes.slice(firstMarkerIndex).trim() : "";
...
function deriveIntegrationTags(
	...
	const keywords: Array<[string, RegExp]> = [
		...
		[
			"Google Workspace",
			/\bgoogle workspace\b|\bgoogle calendar\b|\bgoogle tasks\b|\bgoogle sheets\b/,
		],
		["Stripe", /\bstripe\b/],
...
async function assertParentPageAccessible(
	sdk: Client,
	parentPageId: string,
): Promise<void> {
...
async function getOrCreateDatabase(
	...
	const duplicate = response.results.find((result) => {
		...
		return (
			...
			block.child_database?.title?.trim() === databaseTitle
		);
	});
	...
	if (duplicate) {
		...
		if (!existingDataSourceId) {
			throw new AppError(
				`Existing child database "${databaseTitle}" did not expose a data source ID`,
			);
		}
...
async function assertDataSourceEmpty(
	sdk: Client,
	dataSourceId: string,
): Promise<void> {
	...
	if ((response.results?.length ?? 0) > 0) {
		throw new AppError(
			"Target data source already contains rows; refusing to duplicate-import into it",
		);
	}
}
...
function buildDataSourceProperties(
	...
	return {
		...
		"Pipeline Stage": {
			select: {
				options: buildSelectOptions(
					rows.map((row) => row.pipelineStage),
					{
		...
		"Registry Status": {
			select: {
				options: buildSelectOptions(
					rows.map((row) => row.registryStatus),
					{
						active: "green",
						recent: "blue",
						parked: "gray",
		...
		"Context Quality": {
			select: {
				options: buildSelectOptions(
					rows.map((row) => row.contextQuality),
					{
						full: "green",
						standard: "blue",
						boilerplate: "gray",
		...
		"Integration Tags": {
			multi_select: {
				options: buildMultiSelectOptions(
					rows.flatMap((row) => row.integrationTags),
					{
						Slack: "orange",
						"Claude API": "purple",
						GitHub: "default",
...
function buildPageProperties(
	...
	const properties: Record<string, unknown> = {
		...
		Completion: row.completion
			? { rich_text: toRichText(row.completion) }
			: undefined,
		Readiness: row.readiness
			? { rich_text: toRichText(row.readiness) }
			: undefined,
		Stack: row.stack ? { rich_text: toRichText(row.stack) } : undefined,
		"Source Group": row.sourceGroup
			? { select: { name: row.sourceGroup } }
			: undefined,
		"Local Path": row.localPath
			? { rich_text: toRichText(row.localPath) }
			: undefined,
		"Registry Status": row.registryStatus
			? { select: { name: row.registryStatus } }
			: undefined,
		"Primary Tool": row.primaryTool
			? { select: { name: row.primaryTool } }
			: undefined,
		"Context Quality": row.contextQuality
			? { select: { name: row.contextQuality } }
			: undefined,
...
function buildProjectMarkdown(row: ProjectImportRow): string {
	const lines = [
		...
		row.keyIntegrations
			? `- Key integrations: ${row.keyIntegrations}`
			: "- Key integrations: None noted in the local audit.",
		...
		"## Audit Notes",
		row.auditNotes ||
			"No extra manual-review notes were attached to this row during the current local audit pass.",
	].filter(Boolean);
...
async function verifyImport(
	...
	if (samplePageId) {
		...
		sampleMarkdownPreview = markdown.markdown
			.split("\n")
			.slice(0, 10)
			.join("\n");
	}
