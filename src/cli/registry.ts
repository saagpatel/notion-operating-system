...
import {
	runDestinationsCheckCommand,
	runDestinationsResolveCommand,
	runDoctorCommand,
	runPublishCommand,
...
const commonOptions = {
	live: {
		name: "live",
		description:
			"Run against live Notion instead of staying in a safe planning mode.",
		type: "boolean",
	},
	today: {
		name: "today",
...
export const cliRegistry: CliCommandDefinition[] = [
	{
		...
		options: [
			{
				name: "request",
				description: "Path to a publish request JSON file.",
				type: "string",
				valueName: "path",
			},
			{
				name: "destination",
				description: "Destination alias to publish into.",
				type: "string",
				valueName: "alias",
			},
			{
				name: "file",
				description: "Path to the input markdown or text file.",
				type: "string",
				valueName: "path",
			},
			...
			{
				name: "title",
				description: "Override the destination title.",
				type: "string",
				valueName: "text",
			},
			{
				name: "property",
				description: "Repeatable property override in key=value form.",
				type: "string-array",
				valueName: "key=value",
			},
		],
	...
	},
	{
		name: "doctor",
		description:
			"Verify local setup, Notion connectivity, and destination config.",
		options: [
			{
				name: "json",
		...
		run: async ({ parsed }) =>
			runDoctorCommand({ json: asBoolean(parsed.options.json) }),
	},
	{
		name: "destinations",
		...
		subcommands: [
			...
			},
			{
				...
				options: [
					{
						name: "write",
						description: "Legacy compatibility flag; ignored.",
						type: "boolean",
					},
				],
	...
	},
	{
		...
		subcommands: [
			...
			},
			{
				name: "migrate",
				description:
					"Materialize the legacy single-workspace layout into an explicit default profile.",
				options: [
					{
						name: "write",
						description: "Persist the registry and descriptor files.",
						type: "boolean",
					},
				],
				run: async ({ parsed }) =>
					runProfilesMigrateCommand({ write: asBoolean(parsed.options.write) }),
			},
			{
				name: "export",
				description:
					"Export the active profile into a portable non-secret bundle.",
				options: [
					{
						name: "output",
						description: "Path to the bundle JSON file.",
						type: "string",
						valueName: "path",
						required: true,
					},
				...
				run: async ({ parsed }) =>
					runProfilesExportCommand({ output: asString(parsed.options.output) }),
			},
			{
				name: "diff",
				description:
					"Compare the active profile against another profile or profile bundle.",
				options: [
					{
						name: "against-profile",
						description: "Profile name to compare against.",
						type: "string",
						valueName: "name",
					},
					{
						name: "against-bundle",
						description: "Path to a profile bundle JSON file.",
						type: "string",
						valueName: "path",
					},
					{
						name: "json",
						description: "Emit the diff as JSON.",
						type: "boolean",
					},
				],
				run: async ({ parsed }) =>
			...
			},
			{
				name: "clone",
				description:
					"Create or refresh a profile from another profile's portable config state.",
				options: [
					{
						name: "source",
					...
					{
						name: "label",
						description: "Optional label for the target profile.",
						type: "string",
						valueName: "text",
					},
					{
					...
					{
						name: "write",
						description: "Persist the cloned profile files.",
						type: "boolean",
					},
					{
						name: "json",
						description: "Emit the clone plan as JSON.",
						type: "boolean",
					},
				],
				run: async ({ parsed }) =>
			...
			},
			{
				name: "bootstrap",
				description:
					"Initialize a profile safely from the active setup or a non-secret bundle.",
				options: [
					{
						name: "target",
						description: "Target profile name.",
					...
					{
						name: "from-bundle",
						description: "Optional bundle JSON path to bootstrap from.",
						type: "string",
						valueName: "path",
					},
					{
					...
					{
						name: "write",
						description: "Persist only the missing profile files.",
						type: "boolean",
					},
					{
						name: "json",
						description: "Emit the bootstrap plan as JSON.",
						type: "boolean",
					},
				],
				run: async ({ parsed }) =>
			...
			},
			{
				name: "upgrade",
				description:
					"Preview or apply profile config-version migrations for the active profile.",
				options: [
					{
						name: "write",
						description: "Persist the migrated profile descriptor.",
						type: "boolean",
					},
					{
						name: "json",
						description: "Emit the upgrade plan as JSON.",
						type: "boolean",
					},
				],
				run: async ({ parsed }) =>
			...
			},
			{
				name: "import",
				description:
					"Preview or restore a bundle into a named workspace profile.",
				options: [
					{
						name: "bundle",
						description: "Path to the bundle JSON file.",
					...
					},
					{
						name: "target",
						description:
							"Target profile name. Defaults to the bundle profile name.",
						type: "string",
						valueName: "name",
					},
					{
						name: "write",
						description: "Persist the imported profile files.",
						type: "boolean",
					},
				],
				run: async ({ parsed }) =>
		...
		subcommands: [
			{
				name: "recent",
				description:
					"Show recent completed or failed command runs from the active log directory.",
				options: [
					{
						name: "json",
						description: "Emit recent runs as JSON.",
						type: "boolean",
					},
					{
						name: "limit",
	...
	buildFamily("control-tower", "Operate the project control tower.", [
		buildConfigCommand(
			"sync",
			"Refresh control-tower derived fields and command center.",
		...
		buildConfigCommand(
			"review-packet",
			"Publish or preview the weekly review packet.",
			[
				commonOptions.live,
				commonOptions.today,
				commonOptions.config,
				{
					name: "include-next-phase",
					description: "Include the next-phase brief section.",
					type: "boolean",
				},
			],
			({ parsed }) =>
		...
		buildConfigCommand(
			"phase-closeout",
			"Close a roadmap phase and persist repo memory artifacts.",
			[
				commonOptions.today,
				commonOptions.config,
				{
					name: "phase",
					description: "Specific phase number to close.",
					type: "number",
					valueName: "number",
				},
			],
		...
		buildConfigCommand(
			"views-plan",
			"Print the saved-view sync plan.",
			[localPortfolioViewsConfigOption],
			({ parsed }) =>
		...
		buildConfigCommand(
			"views-validate",
			"Validate the saved-view plan against the live schema.",
			[localPortfolioViewsConfigOption],
			({ parsed }) =>
	...
	buildFamily("execution", "Run execution-system workflows.", [
		...
		),
		{
			name: "views-validate",
			description:
				"Validate the execution saved-view plan against the live schema.",
			run: async () => runExecutionViewsValidateCommand(),
		},
		buildConfigCommand(
			"weekly-plan",
			"Generate or publish the weekly execution planning packet.",
			[
				commonOptions.live,
				commonOptions.today,
				commonOptions.config,
				{
					name: "include-next-phase",
					description: "Include the next-phase brief in the weekly packet.",
					type: "boolean",
				},
			],
			({ parsed }) =>
	...
	buildFamily(
		"intelligence",
		"Run intelligence and recommendation workflows.",
		[
			buildConfigCommand(
				"sync",
			...
			buildConfigCommand(
				"views-validate",
				"Validate the intelligence saved-view plan against the live schema.",
				[commonOptions.config],
				({ parsed }) =>
			...
			buildConfigCommand(
				"recommendation-run",
				"Create a recommendation run record and weekly intelligence summary.",
				[
					commonOptions.live,
					commonOptions.today,
					commonOptions.config,
					{
						name: "type",
						description: "Recommendation run type.",
						type: "enum",
						valueName: "type",
						choices: ["weekly", "daily", "adhoc"],
						defaultValue: "weekly",
					},
				],
				({ parsed }) =>
					runRecommendationRunCommand({
						...
						today: asString(parsed.options.today),
						type:
							(asString(parsed.options.type) as
								| "weekly"
								| "daily"
								| "adhoc"
								| undefined) ?? "weekly",
			...
			buildConfigCommand(
				"link-suggestions-sync",
				"Generate or publish suggested project support links.",
				[commonOptions.live, commonOptions.config],
				({ parsed }) =>
	...
	buildFamily("signals", "Run external-signal and activity workflows.", [
		buildConfigCommand(
			"sync",
			"Sync external provider signals into the Notion system.",
			[
				commonOptions.live,
				commonOptions.today,
				commonOptions.config,
				...
				{
					name: "source-limit",
					description: "Optional maximum number of active sources to sync.",
					type: "number",
					valueName: "count",
				},
				{
					name: "max-events-per-source",
					description: "Optional override for events fetched per source.",
					type: "number",
					valueName: "count",
				},
			],
			({ parsed }) =>
				runExternalSignalSyncCommand({
					...
					today: asString(parsed.options.today),
					provider:
						(asString(parsed.options.provider) as
							| "github"
							| "vercel"
							| "notification_hub"
							| "repo_auditor"
		...
		buildConfigCommand(
			"seed-mappings",
			"Seed external signal source mappings for priority projects.",
			[
				commonOptions.live,
				commonOptions.config,
				{
					name: "limit",
					description: "Maximum number of seed mappings to create.",
					type: "number",
					valueName: "count",
					defaultValue: 15,
				},
		...
		buildConfigCommand(
			"activity-refresh",
			"Refresh latest activity and build evidence on project rows.",
			[
				commonOptions.live,
				commonOptions.config,
				{
					name: "limit",
					description: "Limit the number of changed projects shown in output.",
					type: "number",
					valueName: "count",
					defaultValue: 10,
				},
		...
		buildConfigCommand(
			"views-validate",
			"Validate the external-signal saved-view plan against the live schema.",
			[commonOptions.config],
			({ parsed }) =>
		...
		buildConfigCommand(
			"provider-expansion-audit",
			"Audit whether non-GitHub provider expansion is ready for a bounded pilot.",
			[commonOptions.config],
			({ parsed }) =>
	...
	buildFamily("governance", "Run governance and actuation workflows.", [
		buildConfigCommand(
			"audit",
			"Audit the governance policy and webhook posture.",
			[commonOptions.config],
			({ parsed }) =>
		...
		buildConfigCommand(
			"health-report",
			"Print a compact governance and actuation health snapshot.",
			[commonOptions.config],
			({ parsed }) =>
		...
		buildConfigCommand(
			"views-validate",
			"Validate the governance saved-view plan against the live schema.",
			[commonOptions.config],
			({ parsed }) =>
		...
		buildConfigCommand(
			"action-request-sync",
			"Refresh action-request, governance, and actuation summaries.",
			[commonOptions.live, commonOptions.today, commonOptions.config],
			({ parsed }) =>
		...
		buildConfigCommand(
			"actuation-audit",
			"Audit the GitHub actuation lane and allowlisted target posture.",
			[commonOptions.config],
			({ parsed }) =>
		...
		buildConfigCommand(
			"action-dry-run",
			"Run a dry-run execution preview for one action request.",
			[
				commonOptions.config,
				{
					name: "request",
					description: "Action request page ID.",
					type: "string",
					valueName: "page-id",
					required: true,
				},
		...
		buildConfigCommand(
			"action-runner",
			"Run approved action requests in dry-run or live mode.",
			[
				commonOptions.config,
				{
					name: "request",
					description: "Optional action request page ID override.",
					type: "string",
					valueName: "page-id",
				},
				{
				...
				{
					name: "limit",
					description: "Optional maximum number of requests to execute.",
					type: "number",
					valueName: "count",
				},
			],
			({ parsed }) =>
				runActionRunnerCommand({
					request: asString(parsed.options.request),
					mode:
						(asString(parsed.options.mode) as "dry-run" | "live" | undefined) ??
						"dry-run",
		...
		buildConfigCommand(
			...
			"Drain captured webhook receipts into Notion governance records.",
			[commonOptions.config],
			({ parsed }) =>
		...
		buildConfigCommand(
			"webhook-reconcile",
			"List webhook deliveries that still need reconcile follow-up.",
			[
				commonOptions.config,
				{
					name: "provider",
					...
					type: "enum",
					valueName: "provider",
					choices: ["github", "vercel", "google_calendar"],
					defaultValue: "github",
				},
			],
			({ parsed }) =>
	...
	buildFamily("rollout", "Run project rollout flows.", [
		buildConfigCommand(
			"operational",
			"Classify rollout candidates and optionally run the pilot flow.",
			[
				commonOptions.live,
				commonOptions.today,
				commonOptions.config,
				{
					name: "run-pilot-dry-run",
					description:
						"Run the pilot request in dry-run mode after classification.",
					type: "boolean",
				},
				{
					name: "run-pilot-live",
					description: "Run the pilot request live after classification.",
					type: "boolean",
				},
				{
					name: "approve-pilot",
					description: "Auto-approve the pilot request during live mode.",
					type: "boolean",
				},
			],
			({ parsed }) =>
		...
		buildConfigCommand(
			"cohort",
			"Run the bounded cohort rollout for the selected projects.",
			[
				commonOptions.live,
				commonOptions.today,
				commonOptions.config,
				{
					name: "approve",
					description: "Auto-approve the created requests during live mode.",
					type: "boolean",
				},
				{
					name: "run-dry",
					description: "Run dry-run execution steps after creating requests.",
					type: "boolean",
				},
				{
					name: "run-live",
					description: "Run live execution steps after creating requests.",
					type: "boolean",
				},
				{
					name: "projects",
					description: "Comma-separated cohort project titles.",
					type: "string",
					valueName: "titles",
				},
			],
	...
	buildFamily("maintenance", "Run recurring portfolio maintenance workflows.", [
		buildConfigCommand(
			"weekly-refresh",
			"Run the safe weekly refresh orchestrator.",
			[
				commonOptions.live,
				commonOptions.today,
				commonOptions.config,
				...
				{
					name: "signal-source-limit",
					description:
						"Optional cap for GitHub sources during the weekly external-signal step.",
					type: "number",
					valueName: "count",
				},
				{
					name: "signal-max-events-per-source",
					description:
						"Optional cap for events fetched per GitHub source during the weekly external-signal step.",
					type: "number",
					valueName: "count",
				},
			],
...
export function getCommandHelp(commandPath: string[]): string | undefined {
	...
	return current
		? renderCommandHelp(current, commandPath.slice(0, -1))
		: undefined;
}
