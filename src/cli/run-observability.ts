...
export const COMMAND_WARNING_CATEGORIES = [
	...
	"retry_recovered",
	"stale_data",
	"managed_markdown_patch_skipped",
] as const;
...
export type CommandWarningCategory =
	(typeof COMMAND_WARNING_CATEGORIES)[number];
export type CommandFailureCategory =
	(typeof COMMAND_FAILURE_CATEGORIES)[number];
...
export async function withCommandRunContext<T>(
	...
	await logger.info(
		"command_started",
		buildLifecycleDetails(context, { status: "started" }),
	);
...
export async function logCommandCompleted(): Promise<void> {
	...
	await context.logger.info(
		"command_completed",
		buildLifecycleDetails(context, { status: "completed" }),
	);
}
...
export function mergeCommandSummary(patch: Partial<CommandRunSummary>): void {
	...
	context.summary = normalizeCommandSummary({
		...
		...patch,
		status: mergedStatus,
		warningCategories: mergeUniqueCategories(
			context.summary.warningCategories,
			patch.warningCategories,
		),
		failureCategories: mergeUniqueCategories(
			context.summary.failureCategories,
			patch.failureCategories,
		),
		metadata: {
...
export function incrementCommandSummary<
	K extends Exclude<keyof CommandRunSummary, "mode" | "metadata">,
>(key: K, amount = 1): void {
	const context = storage.getStore();
	if (!context) {
		return;
...
export function recordCommandWarningCategory(
	...
	const warningCategories = mergeUniqueCategories(
		context.summary.warningCategories,
		[category],
	);
	const isNewCategory =
		warningCategories.length !==
		(context.summary.warningCategories?.length ?? 0);
	...
	context.summary = normalizeCommandSummary({
		...
		status: mergeSummaryStatus(
			...
			options.status ??
				(category === "partial_success" ? "partial" : "warning"),
		),
	});
}
...
export function recordCommandFailureCategory(
	...
	const failureCategories = mergeUniqueCategories(
		context.summary.failureCategories,
		[category],
	);
	const isNewCategory =
		failureCategories.length !==
		(context.summary.failureCategories?.length ?? 0);
...
function deriveCommandMode(
	parsed: ParsedCliArgs,
): CommandRunSummary["mode"] | undefined {
...
function mergeUniqueCategories<T extends string>(
	current: T[] | undefined,
	next: T[] | undefined,
): T[] {
...
function normalizeCommandSummary(
	...
	const hasPartialSuccess =
		...
		warningCategories.includes("partial_success");
	const hasSuccessfulWork =
		(summary.recordsCreated ?? 0) > 0 ||
		(summary.recordsUpdated ?? 0) > 0 ||
		(summary.recordsSkipped ?? 0) > 0;
