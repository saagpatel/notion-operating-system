...
const QUEUE_SCORE: Record<
	Exclude<ControlTowerProjectRecord["operatingQueue"], undefined>,
	number
> = {
	Shipped: 0,
	"Needs Review": 0,
...
export async function runOperationalRolloutCommand(
	...
	if (!flags.live) {
		const output = {
			...
			pilotCandidate: plan.pilotCandidate
				? summarizeCandidate(plan.pilotCandidate)
				: undefined,
		};
	...
	const postClassificationBaseline = await runScriptJson(
		"portfolio-audit:control-tower-sync",
		["--live"],
	);

	let pilotRequest:
		| Awaited<ReturnType<typeof ensurePilotActionRequest>>
		| undefined;
	...
	recordCommandOutputSummary(
		...
		},
		{
			...
			warningCategories:
				deriveOperationalRolloutWarningCategories(pilotRunFailures),
			failureCategories:
				deriveOperationalRolloutFailureCategories(pilotRunFailures),
			metadata: {
				shortlistedProjects: plan.wave1Shortlist.length,
			},
		},
...
export function buildOperationalRolloutPlan(input: {
	...
	const moveCandidates = candidates.filter(
		(candidate) => candidate.classification === "move to GitHub next",
	);
	const existingRepoShortlist = moveCandidates.filter((candidate) =>
		[
			...
			"known_repo_allowlisted",
			"known_repo",
		].includes(candidate.githubLane),
	);

	const wave1Shortlist = existingRepoShortlist.slice(0, 2);
	if (wave1Shortlist.length === 0) {
		const seededCandidate = moveCandidates.find(
			(candidate) => candidate.githubLane === "seeded_needs_mapping",
		);
		if (seededCandidate) {
	...
	const wave1ProjectIds = new Set(
		wave1Shortlist.map((candidate) => candidate.projectId),
	);
	const wave2Queue = moveCandidates
		.filter((candidate) => !wave1ProjectIds.has(candidate.projectId))
		.slice(0, 2);

	const wave2ProjectIds = new Set(
		wave2Queue.map((candidate) => candidate.projectId),
	);
	const finalCandidates: OperationalRolloutCandidate[] = candidates.map(
		(candidate) => ({
			...candidate,
	...
	return {
		...
		wave1Shortlist: finalCandidates.filter((candidate) =>
			wave1ProjectIds.has(candidate.projectId),
		),
		wave2Queue: finalCandidates.filter((candidate) =>
			wave2ProjectIds.has(candidate.projectId),
		),
		pilotCandidate:
			finalCandidates.find(
				(candidate) =>
					candidate.projectId ===
					wave1Shortlist.find(
						(entry) => entry.githubLane === "active_allowlisted",
					)?.projectId,
			) ??
			finalCandidates.find((candidate) =>
				wave1ProjectIds.has(candidate.projectId),
			),
	};
}
...
export function classifyOperationalRolloutProject(input: {
	...
	if (
		!input.project.operatingQueue ||
		!PRIORITY_QUEUE_SET.has(input.project.operatingQueue as PriorityQueue)
	) {
		throw new AppError(
			`Project "${input.project.title}" is outside the operational rollout priority slice`,
		);
	}
	...
	const matureForGitHub = [
		"Feature Complete",
		"Demoable",
		"Functional Core",
	].includes(input.project.buildMaturity);
	const shipReadyForGitHub = [
		"Ship-Ready",
		"Near Ship",
		"Needs Hardening",
	].includes(input.project.shipReadiness);
	const locallyOperable = ["Yes", "Partial", "Likely"].includes(
		input.project.runsLocally,
	);
	...
	if (
		input.project.currentState === "Needs Decision" &&
		input.project.shipReadiness === "Not Ready" &&
		highFriction
	) {
		classification = "not worth migrating yet";
		githubLane = seededGitHubRow ? "seeded_not_ready" : "none";
		rationale =
			"High setup friction plus low readiness makes GitHub migration noise right now.";
	} else if (hasActiveMapping && explicitlyAllowlisted) {
		classification = "move to GitHub next";
		githubLane = "active_allowlisted";
		rationale =
			"This project already has an active GitHub source and an explicit actuation target, so it is the safest live pilot.";
	} else if (hasActiveMapping) {
		classification = "move to GitHub next";
		githubLane = "active_known_repo";
		rationale =
			"This project already has an active GitHub repo mapping and can move into the GitHub lane without inventing identifiers.";
	} else if (hasKnownRepo && explicitlyAllowlisted) {
		classification = "move to GitHub next";
		githubLane = "known_repo_allowlisted";
		rationale =
			"The repo identity is already known and allowlisted; activate it after the first live pilot succeeds.";
	} else if (hasKnownRepo) {
		classification = "move to GitHub next";
		githubLane = "known_repo";
		rationale =
			"The repo identity is already known, so this can join the GitHub lane without redesigning the project.";
	} else if (
		seededGitHubRow &&
		matureForGitHub &&
		shipReadyForGitHub &&
		...
		rationale =
			"The project looks mature enough for GitHub next, but the repo mapping still needs to be confirmed.";
	} else {
		classification = "keep Notion-only";
		githubLane = seededGitHubRow ? "seeded_not_ready" : "none";
		rationale =
			"The existing Notion review and execution loop is the better operating surface for this project right now.";
	}
...
function compareCandidatePriority(
	left: OperationalRolloutCandidate,
	right: OperationalRolloutCandidate,
): number {
...
function rankingScore(candidate: OperationalRolloutCandidate): number {
	...
	return (
		classificationScore[candidate.classification] +
		laneScore[candidate.githubLane] +
		(QUEUE_SCORE[candidate.queue] ?? 0) * 25
	);
}
...
function isPriorityProject(project: ControlTowerProjectRecord): boolean {
	return Boolean(
		project.operatingQueue &&
			PRIORITY_QUEUE_SET.has(project.operatingQueue as PriorityQueue),
	);
}
...
function buildRecommendedNextMove(input: {
	...
	const nextMove =
		input.nextMove ||
		"define the next move and keep the project visible in weekly review.";
...
async function runBaselineCommands(
	...
	return {
		...
		controlTowerSync: await runScriptJson(
			"portfolio-audit:control-tower-sync",
			liveArgs,
		),
		reviewPacket: await runScriptJson(
			"portfolio-audit:review-packet",
			liveArgs,
		),
		recommendationRun: await runScriptJson(
			"portfolio-audit:recommendation-run",
			["--type", "weekly", ...liveArgs],
		),
	};
}
...
async function loadRolloutContext(
	...
	if (
		...
		!config.phase6Governance ||
		!config.phase2Execution
	) {
		throw new AppError(
			"Operational rollout requires phases 2, 5, and 6 to be configured",
		);
	}
	...
	const [projectPages, sourcePages, policyPages, requestPages] =
		await Promise.all([
			fetchAllPages(
				sdk,
	...
	return {
		...
		githubSources: sourcePages
			...
			.filter(
				(source) =>
					source.provider === "GitHub" && source.sourceType === "Repo",
			),
...
async function applyOperationalRollout(input: {
	...
	for (const candidate of input.plan.candidates) {
		const project = input.context.projects.find(
		...
		const nextMoveChanged =
			project.nextMove.trim() !== candidate.recommendedNextMove.trim();
		if (nextMoveChanged) {
		...
		if (
			candidate.classification === "keep Notion-only" ||
		...
		await upsertPageByTitle({
			api: input.api,
			dataSourceId:
				input.context.config.phase2Execution!.decisions.dataSourceId,
			titlePropertyName: input.context.decisionTitlePropertyName,
			title: candidate.decisionTitle,
			properties: {
				[input.context.decisionTitlePropertyName]: titleValue(
					candidate.decisionTitle,
				),
				Status: { select: { name: "Committed" } },
				"Decision Type": { select: { name: "Portfolio" } },
				"Decision Owner": peopleValue(
					input.context.config.phase2Execution?.defaultOwnerUserId,
				),
...
async function ensurePilotActionRequest(input: {
	...
	if (!input.pilot.githubSourceId) {
		throw new AppError(
			`Pilot candidate "${input.pilot.projectTitle}" does not have an active GitHub source`,
		);
	}
	...
	return ensureGitHubCreateIssueActionRequest({
		...
		approvalReasonApproved:
			"Approved as the safest-path first live pilot for the operational rollout.",
		approvalReasonPending:
			"Pending operator approval for the first live GitHub pilot.",
		executionNotes:
		...
		markdownPurpose:
			"Validate the first governed live GitHub issue path using the existing Notion operating system.",
	});
}
...
export async function ensureGitHubCreateIssueActionRequest(
	...
	if (!source || source.status !== "Active") {
		throw new AppError(
			`Project "${input.projectTitle}" does not have an active GitHub source`,
		);
	}

	const existing = input.actionRequests.find(
		(entry) => entry.title === input.requestTitle,
	);
	...
	const status =
		existing?.status === "Approved" || input.approve
			? "Approved"
			: "Pending Approval";
	...
	const properties = {
		...
		"Requested By": peopleValue(
			input.config.phase2Execution.defaultOwnerUserId,
		),
		Approver:
			status === "Approved"
				? peopleValue(input.config.phase2Execution.defaultOwnerUserId)
				: { people: [] },
		"Requested At": { date: { start: input.today } },
		"Decided At":
			status === "Approved" ? { date: { start: input.today } } : { date: null },
		"Expires At": {
			date: {
				start: addDays(input.today, policy.defaultExpiryHours >= 48 ? 3 : 2),
			},
		},
	...
	const created = await input.api.createPageWithMarkdown({
		parent: {
			data_source_id: input.config.phase6Governance.actionRequests.dataSourceId,
		},
		properties: {
			[input.actionRequestTitlePropertyName]: titleValue(input.requestTitle),
		},
...
export function buildOperationalRolloutPilotStepGroups(input: {
	...
	return {
		postCreate: [
			{
				key: "requestSyncAfterCreate",
				script: "portfolio-audit:action-request-sync",
				args: ["--live"],
			},
			{
		...
		live: input.runPilotLive
			? [
					...
					{
						key: "requestSyncAfterLive",
						script: "portfolio-audit:action-request-sync",
						args: ["--live"],
					},
					{
						key: "webhookDrain",
						script: "portfolio-audit:webhook-shadow-drain",
						args: [],
					},
					{
						key: "webhookReconcile",
...
export function renderDecisionMarkdown(
	candidate: OperationalRolloutCandidate,
): string {
	return [
	...
	return input.targetConfig.targets.find(
		(target) =>
			...
			(input.source?.identifier &&
				target.sourceIdentifier === input.source.identifier) ||
			(input.source?.sourceUrl && target.sourceUrl === input.source.sourceUrl),
	);
}

export async function runScriptJson(
	script: string,
	args: string[],
): Promise<unknown> {
	...
	return await new Promise((resolve, reject) => {
		...
		child.on("close", (code) => {
			if (code !== 0) {
				reject(
					new AppError(
						`Command "${script}" failed: ${stderr.trim() || stdout.trim()}`,
					),
				);
				return;
			}
