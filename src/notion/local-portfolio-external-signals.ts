...
export type ExternalSignalProviderName =
	| "GitHub"
	| "Vercel"
	| "Google Calendar"
	| "Netlify"
...
export function getPrimarySourceProjectId(
	source: ExternalSignalSourceRecord,
): string | undefined {
	const projectId = source.localProjectIds.find(
...
export function parseLocalPortfolioExternalSignalSourceConfig(
	...
	if (!raw || typeof raw !== "object") {
		throw new AppError(
			"Local portfolio external signal source config must be an object",
		);
	}
	const value = raw as Record<string, unknown>;
	if (value.version !== 1) {
		throw new AppError(
			`Unsupported local portfolio external signal source config version "${String(value.version)}"`,
		);
	}
...
export function parseLocalPortfolioExternalSignalProviderConfig(
	...
	if (!raw || typeof raw !== "object") {
		throw new AppError(
			"Local portfolio external signal provider config must be an object",
		);
	}
	const value = raw as Record<string, unknown>;
	if (value.version !== 1) {
		throw new AppError(
			`Unsupported local portfolio external signal provider config version "${String(value.version)}"`,
		);
	}
...
export function parseLocalPortfolioExternalSignalViewPlan(
	...
	if (!raw || typeof raw !== "object") {
		throw new AppError(
			"Local portfolio external signal views config must be an object",
		);
	}
	const value = raw as Record<string, unknown>;
	if (value.version !== 1) {
		throw new AppError(
			`Unsupported local portfolio external signal views config version "${String(value.version)}"`,
		);
	}
...
export function buildExternalSignalSummary(input: {
	...
	const activeSources = mappedSources.filter(
	...
	const hasDeployment = activeSources.some(
		(source) => source.sourceType === "Deployment Project",
	);
	const hasCalendar = activeSources.some(
		(source) => source.sourceType === "Calendar",
	);

	const coverage = deriveExternalCoverage({
		hasRepo,
		hasDeployment,
		hasCalendar,
		mappedSources,
	...
	const latestDeployment = recentEvents.find(
		(event) => event.signalType === "Deployment",
	);
	const latestDeploymentStatus = deriveLatestDeploymentStatus(
	...
	const openPrCount = recentEvents.filter(
		(event) =>
			...
			["open", "draft", "ready_for_review"].includes(
	...
	const repoActivityFreshness = calculateRepoActivityFreshness(
		recentEvents,
		hasRepo,
		input.today,
	);
	const workflowHealth = calculateWorkflowHealth(
		recentEvents,
		hasRepo,
		input.today,
	);
	const deploymentHealth = calculateDeploymentHealth(
		recentEvents,
		hasDeployment,
		input.today,
	);
	...
	return {
		...
		externalSignalUpdated:
			latestExternalActivity ||
			newestDate(activeSources.map((source) => source.lastSyncedAt)),
		mappedSources,
		activeSources,
		recentEvents,
		repoActivityFreshness,
...
export function buildExternalRecommendationAdjustments(
	...
	const resumeBoost = roundScore(
		summary.repoActivityFreshness * 0.08 +
			summary.workflowHealth * 0.07 +
			coverageBoost,
	);
	const finishBoost = roundScore(
		summary.deploymentHealth * 0.12 +
			summary.workflowHealth * 0.08 +
			coverageBoost,
	);
	...
	const inactivityPenalty = roundScore(
		(100 - summary.repoActivityFreshness) * 0.1 + coverageBoost,
	);
...
export function buildExternalSignalSeedPlans(input: {
	...
	const selectedProjects = selectPriorityProjectsForExternalSignals({
		...
		includePacketPriorities:
			input.sourceConfig.seedRules.includePacketPriorities,
...
export function selectPriorityProjectsForExternalSignals(input: {
	...
	return [...input.projects]
		.filter(
			(project) =>
				input.targetQueues.includes(project.operatingQueue ?? "") ||
				packetProjectIds.has(project.id),
		)
		.sort((left, right) => {
			...
			const queueRank =
				input.targetQueues.indexOf(left.operatingQueue ?? "") -
				input.targetQueues.indexOf(right.operatingQueue ?? "");
			if (queueRank !== 0) {
				return queueRank;
			}
...
export function renderExternalSignalBriefSection(input: {
	...
	const recentEvents = [...input.summary.recentEvents]
		.sort(compareRecentSignalEvents)
		.slice(0, 5);
	const mappedSources = [...input.summary.mappedSources].sort(
		compareMappedSources,
	);
	...
	return [
		...
		...(recentEvents.length > 0
			? recentEvents.map(
					(event) =>
						`- [${event.title}](${event.url}) - ${event.signalType} / ${event.status} / ${event.occurredAt}`,
				)
...
export function renderExternalSignalCommandCenterSection(
	input: ExternalSignalCommandCenterInput,
): string {
	const contradictionProjects = input.summaries
	...
	const projectsNeedingMapping = input.summaries
		...
		.map(
			(summary) =>
				input.projects.find((project) => project.id === summary.projectId)
	...
	const recentHealthyDeploys = input.summaries
		...
		.sort((left, right) =>
			right.latestExternalActivity.localeCompare(left.latestExternalActivity),
		)
		.slice(0, 5);
	...
	return [
		...
		...(recentHealthyDeploys.length > 0
			? recentHealthyDeploys.map(
					(summary) =>
						`- ${projectTitle(input.projects, summary.projectId)} - ${summary.latestExternalActivity}`,
				)
		...
		...(recentWorkflowFailures.length > 0
			? recentWorkflowFailures.map(
					(summary) =>
						`- ${projectTitle(input.projects, summary.projectId)} - ${summary.recentFailedWorkflowRuns} recent failed runs`,
				)
...
export function renderWeeklyExternalSignalsSection(input: {
	...
	return [
		...
		...(wins.length > 0
			? wins.map(
					(summary) =>
						`- ${summary.projectId} has a recent successful deployment signal.`,
				)
		...
		...(failures.length > 0
			? failures.map(
					(summary) =>
						`- ${summary.projectId} has ${summary.recentFailedWorkflowRuns} recent failed workflow runs.`,
				)
			: ["- No workflow or deployment regressions captured yet."]),
		"",
		"### Coverage Gaps",
		...(input.summaries.filter((summary) => summary.coverage === "None")
			.length > 0
			? input.summaries
					.filter((summary) => summary.coverage === "None")
					.slice(0, 5)
					.map(
						(summary) =>
							`- ${summary.projectId} still needs real external-source mappings.`,
					)
		...
		...(contradictions.length > 0
			? contradictions.map(
					(summary) =>
						`- ${summary.projectId} should be re-reviewed because telemetry contradicts the current Notion posture.`,
				)
			: [
		...
		...(latestSyncRuns.length > 0
			? latestSyncRuns.map(
					(run) => `- [${run.title}](${run.url}) - ${run.status}`,
				)
			: ["- No sync runs yet."]),
...
function compareMappedSources(
	left: ExternalSignalSourceRecord,
	right: ExternalSignalSourceRecord,
): number {
	return (
...
function compareRecentSignalEvents(
	left: ExternalSignalEventRecord,
	right: ExternalSignalEventRecord,
): number {
	return (
...
function compareWins(
	left: ExternalSignalSummary,
	right: ExternalSignalSummary,
): number {
	return (
		compareDescending(
			left.latestExternalActivity,
			right.latestExternalActivity,
...
function compareContradictionSummaries(
	left: ExternalSignalSummary,
	right: ExternalSignalSummary,
): number {
	return (
...
export function calculateExternalSignalMetrics(input: {
	...
	return {
		mappedProjects: input.summaries.filter(
			(summary) => summary.coverage !== "None",
		).length,
		projectsNeedingMapping: input.summaries.filter(
			(summary) => summary.coverage === "None",
		).length,
		activeSources: input.summaries.reduce(
			(sum, summary) => sum + summary.activeSources.length,
			0,
		),
		riskEvents: input.summaries.reduce(
			(sum, summary) =>
				sum +
				summary.recentEvents.filter((event) => event.severity === "Risk")
					.length,
			0,
		),
		successfulDeployments: input.summaries.filter(
			(summary) => summary.latestDeploymentStatus === "Success",
		).length,
		failedWorkflowRuns: input.summaries.reduce(
			(sum, summary) => sum + summary.recentFailedWorkflowRuns,
			0,
		),
		contradictionProjects: input.summaries.filter(
			(summary) => summary.contradictionLabel === "Contradicts",
		).length,
	};
}
...
export function validateLocalPortfolioExternalSignalViewPlanAgainstSchemas(input: {
	...
	schemas: Record<
		ExternalSignalViewCollection["key"],
	...
	for (const collection of input.plan.collections) {
		...
		if (!schema) {
			throw new AppError(
				`Missing schema for external signal view collection "${collection.key}"`,
			);
		}
...
export function normalizeProviderKey(
	value: ExternalSignalProviderName | ExternalProviderKey,
): ExternalProviderKey {
	switch (value) {
		case "GitHub":
		case "github":
...
function parseSourceStrategy(
	...
	if (value.primary !== "direct_rest" || value.fallback !== "manual_review") {
		throw new AppError(
			"External signal source config strategy must be direct_rest/manual_review",
		);
	}
	return {
		primary: "direct_rest",
		...
		notes: requiredStringArray(
			value.notes,
			"externalSignalSources.strategy.notes",
		),
	};
}
...
function parseSeedRules(
	...
	const value = raw as Record<string, unknown>;
	return {
		targetQueues: requiredStringArray(
			value.targetQueues,
			"externalSignalSources.seedRules.targetQueues",
		),
		...
		limit: requiredPositiveNumber(
			value.limit,
			"externalSignalSources.seedRules.limit",
		),
	};
}
...
function parseSourceTemplates(
	...
	if (!Array.isArray(raw) || raw.length === 0) {
		throw new AppError(
			"External signal source config must include sourceTemplates",
		);
	}
	...
	return raw.map((entry, index) => {
		if (!entry || typeof entry !== "object") {
			throw new AppError(
				`externalSignalSources.sourceTemplates[${index}] must be an object`,
			);
		}
		const value = entry as Record<string, unknown>;
		return {
			provider: parseProviderName(
				requiredString(value.provider, `sourceTemplates[${index}].provider`),
			),
			sourceType: parseSourceType(
				requiredString(
					value.sourceType,
					`sourceTemplates[${index}].sourceType`,
				),
			),
			titleSuffix: requiredString(
				value.titleSuffix,
				`sourceTemplates[${index}].titleSuffix`,
			),
			defaultStatus: parseSourceStatus(
				requiredString(
					value.defaultStatus,
					`sourceTemplates[${index}].defaultStatus`,
				),
			),
			defaultEnvironment: parseEnvironment(
				requiredString(
					value.defaultEnvironment,
					`sourceTemplates[${index}].defaultEnvironment`,
				),
			),
			defaultSyncStrategy: parseSyncStrategy(
				requiredString(
					value.defaultSyncStrategy,
					`sourceTemplates[${index}].defaultSyncStrategy`,
				),
			),
		};
	});
...
function parseManualSeeds(raw: unknown): ManualExternalSignalSeedPlan[] {
	...
	if (!Array.isArray(raw)) {
		throw new AppError(
			"External signal source config manualSeeds must be an array",
		);
	}
	...
	return raw.map((entry, index) => {
		if (!entry || typeof entry !== "object") {
			throw new AppError(
				`externalSignalSources.manualSeeds[${index}] must be an object`,
			);
		}
		const value = entry as Record<string, unknown>;
		const parsed = {
			...
			sourceType: parseSourceType(
				requiredString(value.sourceType, `manualSeeds[${index}].sourceType`),
			),
			status: parseSourceStatus(
				requiredString(value.status, `manualSeeds[${index}].status`),
			),
			environment: parseEnvironment(
				requiredString(value.environment, `manualSeeds[${index}].environment`),
			),
			syncStrategy: parseSyncStrategy(
				requiredString(
					value.syncStrategy,
					`manualSeeds[${index}].syncStrategy`,
				),
			),
			identifier: optionalString(value.identifier),
			sourceUrl: optionalString(value.sourceUrl),
			providerScopeType: optionalProviderScopeType(
				value.providerScopeType,
				`manualSeeds[${index}].providerScopeType`,
			),
		...
		if (parsed.provider === "Vercel") {
			if (!parsed.identifier?.trim()) {
				throw new AppError(
					`manualSeeds[${index}].identifier is required for Vercel sources`,
				);
			}
			if (!parsed.sourceUrl?.trim()) {
				throw new AppError(
					`manualSeeds[${index}].sourceUrl is required for Vercel sources`,
				);
			}
			if (!parsed.providerScopeType) {
				throw new AppError(
					`manualSeeds[${index}].providerScopeType is required for Vercel sources`,
				);
			}
			if (
				parsed.providerScopeType === "Team" &&
				!parsed.providerScopeId?.trim()
			) {
				throw new AppError(
					`manualSeeds[${index}].providerScopeId is required for Vercel sources`,
				);
			}
			if (
				parsed.providerScopeType === "Team" &&
				!parsed.providerScopeSlug?.trim()
			) {
				throw new AppError(
					`manualSeeds[${index}].providerScopeSlug is required for Vercel sources`,
				);
			}
		}
		return parsed;
...
function parseProviderPlans(raw: unknown): ExternalSignalProviderPlan[] {
	if (!Array.isArray(raw) || raw.length === 0) {
		throw new AppError(
			"External signal provider config must include providers",
		);
	}
	...
	return raw.map((entry, index) => {
		if (!entry || typeof entry !== "object") {
			throw new AppError(
				`externalSignalProviders.providers[${index}] must be an object`,
			);
		}
		const value = entry as Record<string, unknown>;
		return {
			...
			authEnvVar: requiredString(
				value.authEnvVar,
				`providers[${index}].authEnvVar`,
			),
			baseUrl: optionalString(value.baseUrl) ?? "",
			syncStrategy: parseProviderSyncStrategy(
				requiredString(value.syncStrategy, `providers[${index}].syncStrategy`),
			),
			sourceTypes: parseProviderSourceTypes(
				value.sourceTypes,
				`providers[${index}].sourceTypes`,
			),
...
function optionalProviderScopeType(
	value: unknown,
	fieldName: string,
): ExternalProviderScopeType | undefined {
	...
	if (entry !== "Personal" && entry !== "Team") {
		throw new AppError(
			`${fieldName} must be "Personal" or "Team" when provided`,
		);
	}
	return entry;
}
...
function parseViewStrategy(
	...
	if (value.primary !== "notion_mcp" || value.fallback !== "playwright") {
		throw new AppError(
			"External signal views config strategy must be notion_mcp/playwright",
		);
	}
	...
	return {
		...
		notes: requiredStringArray(
			value.notes,
			"externalSignalViews.strategy.notes",
		),
	};
}

function parseViewCollections(raw: unknown): ExternalSignalViewCollection[] {
	if (!Array.isArray(raw) || raw.length === 0) {
	...
	return raw.map((entry, index) => {
		if (!entry || typeof entry !== "object") {
			throw new AppError(
				`externalSignalViews.collections[${index}] must be an object`,
			);
		}
		const value = entry as Record<string, unknown>;
		const key = requiredString(
			value.key,
			`externalSignalViews.collections[${index}].key`,
		);
		if (
			key !== "sources" &&
			key !== "events" &&
		...
		return {
			key,
			database: parseExternalSignalDatabaseRef(
				value.database,
				`externalSignalViews.collections[${index}].database`,
			),
			views: parseExternalSignalViews(
				value.views,
				`externalSignalViews.collections[${index}].views`,
			),
		};
	});
}
...
function parseExternalSignalViews(
	raw: unknown,
	fieldName: string,
): ExternalSignalViewSpec[] {
	if (!Array.isArray(raw) || raw.length === 0) {
...
function calculateRepoActivityFreshness(
	...
	const latestRepoEvent = recentEvents.find(
...
function calculateWorkflowHealth(
	...
	const workflowEvents = recentEvents.filter(
		(event) => event.signalType === "Workflow Run",
	);
	...
	const recentFailures = workflowEvents.filter(
		(event) =>
			isFailureStatus(event.status) && diffDays(event.occurredAt, today) <= 14,
	).length;
	...
	return clamp(
		88 -
			recentFailures * 22 -
			(latest && isFailureStatus(latest.status) ? 18 : 0),
	);
}
...
function calculateDeploymentHealth(
	...
	const deploymentEvents = recentEvents.filter(
		(event) => event.signalType === "Deployment",
	);
	...
	if (
		["building", "queued", "in_progress"].includes(latest.status.toLowerCase())
	) {
...
function renderRecommendationImpact(summary: ExternalSignalSummary): string[] {
	if (summary.coverage === "None") {
		return [
			"- External telemetry is not active yet, so recommendations still rely on Notion-native signals only.",
		];
	}
	if (summary.contradictionLabel === "Contradicts") {
		return [
			"- External telemetry is contradicting the optimistic Notion posture and should increase review urgency.",
		];
	}
	if (
		summary.latestDeploymentStatus === "Success" ||
		summary.repoActivityFreshness >= 60
	) {
		return [
			"- External telemetry is reinforcing the active recommendation with fresh repo or deployment evidence.",
		];
	}
	return [
		"- External telemetry is additive but not yet strong enough to materially change the recommendation.",
	];
}
...
function parseProviderSourceTypes(
	value: unknown,
	fieldName: string,
): ExternalSourceType[] {
...
export function defaultSyncRunScope(
	provider: ExternalSignalProviderName,
	sourceCount: number,
): string {
