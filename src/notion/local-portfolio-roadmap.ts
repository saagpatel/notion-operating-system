...
export function buildRoadmapPhases(
	...
	const phase1Status =
		currentPhase > 1 || phaseOneCompleted
			? "Completed"
			: currentPhase === 1 &&
					currentPhaseStatus.toLowerCase().includes("progress")
	...
	return [
		...
		{
			phase: 10,
			title: "Signal Wiring and Intelligence Layer",
			...
			exitCriteria: [
				...
				"Typecheck clean, 242+ tests pass",
			],
			nextPhaseBrief:
				"Phase 10C should upgrade the morning brief to use Claude API for a synthesis paragraph per top-priority project, wire approved orphan dispositions into work_packets creation, and pipe trend analysis output into the weekly review page as a managed section.",
		},
	];
}
...
export function renderNotionRoadmapMarkdown(input: {
	...
	const phases = buildRoadmapPhases(
		...
		input.currentPhase > 1 ||
			input.currentPhaseStatus.toLowerCase().includes("complete"),
	);
	const currentPhaseEntry =
		phases.find((phase) => phase.phase === input.currentPhase) ?? phases[0]!;
	...
	const nextPhaseEntry =
		...
		currentPhaseEntry;
	const nextPhaseBrief = currentPhaseEntry.nextPhaseBrief;
	...
	const lines = [
		...
		...renderMetricSection(
			...
			"Baseline metrics will be captured on the first live control-tower sync.",
		),
		"",
		"## Latest Metrics",
		...renderMetricSection(
			input.latestMetrics,
			"Latest metrics are not captured yet.",
		),
	];

	if (
		previousPhaseEntry &&
		previousPhaseEntry.phase + 1 === currentPhaseEntry.phase
	) {
		lines.push(
			"",
...
function renderMetricSection(
	metrics: ControlTowerMetrics | undefined,
	emptyState: string,
): string[] {
	if (!metrics) {
		return [`- ${emptyState}`];
