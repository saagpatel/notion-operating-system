		...
		sdk.request({
			...
			body: {
				properties: {
					...
					Supersedes: {
						relation: relationSchema(recommendationRuns.dataSourceId),
					},
					Reviewer: { people: {} },
					"Reviewed On": { date: {} },
					Summary: { rich_text: {} },
		...
		sdk.request({
			...
			body: {
				properties: {
					...
					"Suggested Research": {
						relation: relationSchema(config.relatedDataSources.researchId),
					},
					"Suggested Skill": {
						relation: relationSchema(config.relatedDataSources.skillsId),
					},
					"Suggested Tool": {
						relation: relationSchema(config.relatedDataSources.toolsId),
					},
					...
					Supersedes: {
						relation: relationSchema(linkSuggestions.dataSourceId),
					},
				},
			},
		}),
...
export function toIntelligenceProjectRecord(
	...
	return {
		...
		buildSessionCount: rollupCountValue(page.properties["Build Session Count"]),
		relatedResearchCount: rollupCountValue(
			page.properties["Related Research Count"],
		),
		supportingSkillsCount: rollupCountValue(
			page.properties["Supporting Skills Count"],
		),
		linkedToolCount: rollupCountValue(page.properties["Linked Tool Count"]),
		...
		operatingQueue: selectValue(
			page.properties["Operating Queue"],
		) as IntelligenceProjectRecord["operatingQueue"],
		nextReviewDate: dateValue(page.properties["Next Review Date"]),
		evidenceFreshness: selectValue(
			page.properties["Evidence Freshness"],
		) as IntelligenceProjectRecord["evidenceFreshness"],
		...
		) as IntelligenceProjectRecord["externalSignalCoverage"],
		latestExternalActivity: dateValue(
			page.properties["Latest External Activity"],
		),
		latestDeploymentStatus: selectValue(
			page.properties["Latest Deployment Status"],
		) as IntelligenceProjectRecord["latestDeploymentStatus"],
		openPrCount: readNumber(page.properties["Open PR Count"]),
		recentFailedWorkflowRuns: readNumber(
			page.properties["Recent Failed Workflow Runs"],
		),
		externalSignalUpdated: dateValue(
			page.properties["External Signal Updated"],
		),
		recommendationLane: selectValue(
			page.properties["Recommendation Lane"],
		) as IntelligenceProjectRecord["recommendationLane"],
		...
		) as IntelligenceProjectRecord["recommendationConfidence"],
		recommendationUpdated: dateValue(page.properties["Recommendation Updated"]),
	};
}

export function toResearchLibraryRecord(
	page: DataSourcePageRef,
): ResearchLibraryRecord {
	return {
		id: page.id,
...
export function toSkillLibraryRecord(
	page: DataSourcePageRef,
): SkillLibraryRecord {
	return {
		id: page.id,
...
export function toRecommendationRunRecord(
	...
	return {
		...
		topInvestigateProjectIds: relationIds(
			page.properties["Top Investigate Project"],
		),
...
export function toLinkSuggestionRecord(
	page: DataSourcePageRef,
): LinkSuggestionRecord {
	return {
		id: page.id,
		...
		title: page.title,
		status: selectValue(page.properties.Status),
		suggestionType: selectValue(
			page.properties["Suggestion Type"],
		) as LinkSuggestionRecord["suggestionType"],
