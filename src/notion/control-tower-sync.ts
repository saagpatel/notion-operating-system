...
export async function runControlTowerSyncCommand(
	...
	const phaseStateUpdate = buildNextControlTowerPhaseState(
		config.phaseState,
		metrics,
		today,
	);
	...
	if (live) {
		...
		const finalConfig = {
			...
			commandCenter: {
				...
				pageUrl:
					commandCenterSummary.pageUrl ?? nextConfig.commandCenter.pageUrl,
			},
		};
		...
		if (
			...
			await registry.patchDestination(
				finalConfig.destinations.commandCenterAlias,
				{
...
export function buildDerivedPropertyUpdates(
	...
	if (previous?.nextReviewDate !== next.nextReviewDate) {
		updates["Next Review Date"] = next.nextReviewDate
			? datePropertyValue(next.nextReviewDate)
			: { date: null };
	}
...
export function countControlTowerChangedRows(
	...
	return nextProjects.filter((project) => {
		...
		return (
			Object.keys(buildDerivedPropertyUpdates(previous, project)).length > 0
		);
	}).length;
}
...
export function buildNextControlTowerPhaseState(
	...
	return {
		...
		phaseState: {
			...phaseState,
			baselineCapturedAt: baselineCaptured
				? today
