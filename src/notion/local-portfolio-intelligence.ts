...
import { parseExecutionDataSource, type ExecutionDataSourceRef } from "./local-portfolio-execution.js";

export const DEFAULT_LOCAL_PORTFOLIO_INTELLIGENCE_VIEWS_PATH = "./config/local-portfolio-intelligence-views.json";

export type RecommendationLane = "Resume" | "Finish" | "Investigate" | "Defer" | "Monitor";
...
export async function loadLocalPortfolioIntelligenceViewPlan(
  filePath = DEFAULT_LOCAL_PORTFOLIO_INTELLIGENCE_VIEWS_PATH,
): Promise<LocalPortfolioIntelligenceViewPlan> {
  const raw = await readJsonFile<unknown>(filePath);
  return parseLocalPortfolioIntelligenceViewPlan(raw);
}

export function parseLocalPortfolioIntelligenceViewPlan(raw: unknown): LocalPortfolioIntelligenceViewPlan {
  if (!raw || typeof raw !== "object") {
    throw new AppError("Local portfolio intelligence views config must be an object");
  }

  const plan = raw as Record<string, unknown>;
  if (plan.version !== 1) {
    throw new AppError(`Unsupported local portfolio intelligence views config version "${String(plan.version)}"`);
  }
...
export function buildProjectIntelligenceContext(input: {
  ...
  const linkedResearch = input.researchRecords.filter((record) => input.project.relatedResearchIds.includes(record.id));
  const linkedSkills = input.skillRecords.filter((record) => input.project.supportingSkillIds.includes(record.id));
  const linkedTools = input.toolRecords.filter((record) => input.project.toolStackIds.includes(record.id));
  const openDecisions = input.decisions.filter(
    (decision) => decision.localProjectIds.includes(input.project.id) && decision.status === "Proposed",
  );
  const projectPackets = input.packets.filter((packet) => packet.localProjectIds.includes(input.project.id));
  const projectTasks = input.tasks.filter((task) => task.localProjectIds.includes(input.project.id));
  const recentBuildSessions = input.buildSessions
    ...
    .slice(0, 5);
  const activePacket = projectPackets.find((packet) => packet.priority === "Now" && !isClosedPacket(packet.status));
  const supportRich = linkedResearch.length + linkedSkills.length + linkedTools.length >= 3;
...
export function buildRecommendation(
  ...
  const external = buildExternalRecommendationAdjustments(context.project, externalSignalSummary);
  const scores = {
    Resume: roundScore(
      context.factors.executionReadiness * 0.3 +
        ...
        freshnessMomentumScore(context.project) * 0.1 +
    ...
    Investigate: roundScore(
      context.factors.decisionStateFit * 0.3 +
        ...
        context.factors.supportFit * 0.2 +
        freshnessMomentumScore(context.project) * 0.15 +
  ...
  const [topLane, topScore] = sorted[0] as [Exclude<RecommendationLane, "Monitor">, number];
  ...
  return {
    ...
    topPositiveFactors,
    limitingFactors,
    whyNow: topPositiveFactors.join("; ") || "The structured signals point to a clear next lane.",
    whyNotNow: limitingFactors.join("; ") || "No major structural constraint is standing out.",
...
export function calculateIntelligenceMetrics(input: {
  ...
  for (const recommendation of input.recommendations) {
    laneCounts.set(recommendation.lane, (laneCounts.get(recommendation.lane) ?? 0) + 1);
  }
  ...
  return {
    ...
    orphanedProjects: input.projects.filter((project) => totalSupportLinks(project) === 0).length,
    supportGapProjects: input.projects.filter((project) => totalSupportLinks(project) < 2).length,
  ...
  for (const project of input.projects) {
    const rejectedKeys = new Set(
      input.existingSuggestions
        .filter((suggestion) => suggestion.status === "Rejected" && suggestion.localProjectIds.includes(project.id))
    ...
    candidates.push(
      ...buildSupportCandidates({
        ...
        minimum,
        rejectedKeys,
        tokenBuilder: (record) => [record.title, record.category, ...record.tags],
      }),
      ...buildSupportCandidates({
        project,
        ...
        minimum,
        rejectedKeys,
        tokenBuilder: (record) => [record.title, record.category, record.projectRelevance, record.status],
      }),
      ...buildSupportCandidates({
        project,
        ...
        minimum,
        rejectedKeys,
        tokenBuilder: (record) => [record.title, record.category, record.stackIntegration, record.status, ...record.tags],
      }),
    );
  }
...
export function renderRecommendationBriefSection(input: {
  ...
  const strongestResearch = input.context.linkedResearch.slice(0, 3).map((record) => `[${record.title}](${record.url})`);
  const strongestSkills = input.context.linkedSkills.slice(0, 3).map((record) => `[${record.title}](${record.url})`);
  const strongestTools = input.context.linkedTools.slice(0, 3).map((record) => `[${record.title}](${record.url})`);

  return [
    "<!-- codex:notion-recommendation-brief:start -->",
    "## Recommendation Brief",
    "",
    `Updated: ${input.context.project.recommendationUpdated || input.context.project.dateUpdated || "Unknown"}`,
    ...
    ...(strongestResearch.length > 0 ? strongestResearch.map((item) => `- ${item}`) : ["- No linked research support yet."]),
    "",
    "### Strongest Skill Support",
    ...(strongestSkills.length > 0 ? strongestSkills.map((item) => `- ${item}`) : ["- No linked skill support yet."]),
    "",
    "### Strongest Tool Support",
    ...(strongestTools.length > 0 ? strongestTools.map((item) => `- ${item}`) : ["- No linked tool support yet."]),
    "",
    "### Missing Support",
...
export function renderIntelligenceCommandCenterSection(input: {
  ...
  const investigate = topRecommendations(input.recommendations, "Investigate", 5);
  ...
  return [
    ...
    "",
    "### Top Investigate",
    ...formatRecommendationBullets(investigate, "- No investigate candidates yet."),
    "",
    "### Top Defer",
...
export function renderWeeklyIntelligenceSection(input: {
  ...
  return [
    ...
    ...formatRecommendationBullets(input.recommendations.slice(0, 4), "- No recommendation changes captured yet."),
    "",
    "### Accepted Link Suggestions",
    ...(input.acceptedSuggestions.length > 0
      ? input.acceptedSuggestions.map((suggestion) => `- [${suggestion.title}](${suggestion.url})`)
      : ["- No accepted link suggestions this week."]),
    "",
    "### Rejected Link Suggestions",
    ...(input.rejectedSuggestions.length > 0
      ? input.rejectedSuggestions.map((suggestion) => `- [${suggestion.title}](${suggestion.url})`)
...
export function renderRecommendationRunMarkdown(input: {
  ...
  return [
    ...
    "",
    "## Portfolio Calls",
    ...(input.topResume ? [`- Resume: ${input.topResume.projectTitle} (${input.topResume.score}, ${input.topResume.confidence})`] : ["- Resume: none"]),
    ...(input.topFinish ? [`- Finish: ${input.topFinish.projectTitle} (${input.topFinish.score}, ${input.topFinish.confidence})`] : ["- Finish: none"]),
    ...(input.topInvestigate ? [`- Investigate: ${input.topInvestigate.projectTitle} (${input.topInvestigate.score}, ${input.topInvestigate.confidence})`] : ["- Investigate: none"]),
    ...(input.topDefer ? [`- Defer: ${input.topDefer.projectTitle} (${input.topDefer.score}, ${input.topDefer.confidence})`] : ["- Defer: none"]),
    "",
    "## Ranked Recommendations",
    ...input.recommendations.slice(0, 10).map(
      (recommendation, index) =>
        `${index + 1}. ${recommendation.projectTitle} - ${recommendation.lane} (${recommendation.score}, ${recommendation.confidence})`,
    ),
    "",
    "## Why These Calls",
    ...input.recommendations.slice(0, 5).flatMap((recommendation) => [
...
function calculateFactors(input: {
  ...
  const linkedSupport = input.linkedResearch.length + input.linkedSkills.length + input.linkedTools.length;
  const blockedPackets = input.projectPackets.filter((packet) => packet.status === "Blocked").length;
  const blockedTasks = input.projectTasks.filter((task) => task.status === "Blocked").length;
  const activePacket = input.projectPackets.find((packet) => packet.priority === "Now" && !isClosedPacket(packet.status));
  const recentDonePackets = input.projectPackets.filter(
    (packet) => packet.status === "Done" && diffDays(packet.targetFinish, input.today) <= 21,
  ).length;
  const rolloutPain = input.projectPackets.reduce((total, packet) => total + packet.rolloverCount, 0) + blockedTasks;
  ...
  const executionReadiness = clamp(
    queueReadiness(input.project.operatingQueue) +
      ...
      inverseOptionScore(input.project.setupFriction, { Low: 20, Medium: 12, High: 4 }) +
  ...
  const evidenceStrength = clamp(
    optionScore(input.project.evidenceFreshness, { Fresh: 35, Aging: 20, Stale: 8 }) +
      optionScore(input.project.evidenceConfidence, { High: 20, Medium: 12, Low: 5 }) +
      optionScore(input.project.docsQuality, { Strong: 15, Usable: 10, Thin: 5, Missing: 0 }) +
      optionScore(input.project.testPosture, { Strong: 15, Some: 10, Sparse: 5, Unknown: 3 }) +
  ...
  const supportFit = clamp(
    Math.min(input.linkedResearch.length, 3) * 12 +
      ...
      optionScore(mostCommon(input.linkedSkills.map((skill) => skill.projectRelevance)), {
        Core: 20,
        Useful: 12,
        Peripheral: 6,
      }) +
  ...
  const attentionCost = clamp(
    inverseOptionScore(input.project.setupFriction, { Low: 40, Medium: 24, High: 8 }) +
  ...
  const coldnessDrift = clamp(
    optionScore(input.project.momentum, { Cold: 30, Warm: 18, Hot: 8 }) +
      optionScore(input.project.evidenceFreshness, { Stale: 25, Aging: 12, Fresh: 5 }) +
      optionScore(input.project.currentState, { Parked: 20, Archived: 25, "Needs Decision": 12, "Active Build": 5 }) +
      boolScore(!input.project.lastActive || diffDays(input.project.lastActive, input.today) > 30, 20),
  );
  const decisionStateFit = clamp(
    optionScore(input.project.currentState, { "Needs Decision": 45, "Ready for Review": 18, "Active Build": 10 }) +
      Math.min(input.openDecisions.length, 3) * 15 +
      Math.min(input.linkedResearch.length, 2) * 10,
  );
  const ambiguityPenalty = clamp(
    100 -
      optionScore(input.project.evidenceConfidence, { High: 70, Medium: 45, Low: 10 }) -
      Math.min(input.linkedResearch.length, 2) * 10,
  );
  const blockerPenalty = clamp(blockedPackets * 40 + blockedTasks * 20 + input.openDecisions.length * 15);
...
function describePositiveFactors(lane: RecommendationLane, context: ProjectIntelligenceContext): string[] {
...
function describeLimitingFactors(lane: RecommendationLane, context: ProjectIntelligenceContext): string[] {
  const factorPairs = factorDescriptions(lane, context, true).sort((left, right) => right.score - left.score);
  return factorPairs.map((entry) => entry.description);
}
...
function factorDescriptions(
  ...
  if (lane === "Finish") {
    return limiting
      ? [
          { score: context.factors.blockerPenalty, description: "Blockers still reduce finish confidence." },
          { score: 100 - shared.evidence, description: "Evidence is not yet as strong as a finish push would like." },
          { score: 100 - shared.support, description: "Support coverage is still thinner than ideal for a finish push." },
        ]
      : [
          { score: shared.finish, description: "The project is already close to a demo or ship point." },
          { score: shared.health, description: "Execution history is healthy enough to support a finish push." },
          { score: shared.evidence, description: "Evidence is fresh enough to justify finishing now." },
        ];
  }
  ...
  if (lane === "Investigate") {
    return limiting
      ? [
          { score: context.factors.ambiguityPenalty, description: "Ambiguity is still high enough that the decision needs more structure." },
          { score: 100 - shared.support, description: "Support coverage is still too thin in at least one domain." },
          { score: 100 - shared.evidence, description: "Evidence freshness or confidence still needs strengthening." },
        ]
      : [
          { score: shared.decisions, description: "The project is already near a meaningful portfolio or delivery decision." },
          { score: shared.evidence, description: "There is enough evidence to investigate the decision cleanly." },
          { score: shared.support, description: "Linked support gives the investigation something concrete to work from." },
        ];
  }
  ...
  if (lane === "Defer") {
    return limiting
      ? [
          { score: 100 - shared.drift, description: "The coldness signal is not overwhelming yet." },
          { score: 100 - context.factors.highFrictionPenalty, description: "Execution friction is not the main problem yet." },
          { score: 100 - context.factors.lowEvidencePenalty, description: "Evidence quality may still justify keeping this alive." },
        ]
      : [
          { score: shared.drift, description: "The project is drifting cold or stale relative to the rest of the portfolio." },
          { score: context.factors.lowEvidencePenalty, description: "Evidence quality is too weak to justify active attention." },
          { score: context.factors.highFrictionPenalty, description: "Attention cost is high compared with the current portfolio upside." },
        ];
  }

  if (lane === "Monitor") {
    return limiting
      ? [{ score: 60, description: "The signals do not yet support a stronger lane than Monitor." }]
      : [{ score: 55, description: "The project should stay visible while stronger evidence accumulates." }];
  }

  return limiting
    ? [
        { score: 100 - shared.readiness, description: "Execution readiness still has a few weak spots." },
        { score: 100 - shared.support, description: "Support coverage is not yet as deep as the strongest candidates." },
        { score: 100 - shared.health, description: "Execution history still carries some friction or blocker drag." },
      ]
    : [
        { score: shared.readiness, description: "The project is ready to restart with low setup friction." },
        { score: shared.support, description: "Support coverage is already good enough to move with confidence." },
        { score: shared.health, description: "Execution history is healthy enough to make progress stick." },
      ];
}

function buildNextAction(lane: RecommendationLane, context: ProjectIntelligenceContext): string {
  switch (lane) {
    case "Resume":
...
function buildSupportCandidates<T extends { id: string; title: string }>(input: {
  project: IntelligenceProjectRecord;
  ...
  return input.records
    ...
    .map((record) => {
      ...
      const recency = recencyBoost(record as Partial<ResearchLibraryRecord & SkillLibraryRecord & ToolMatrixRecord>);
      const needBoost = suggestionNeedBoost(input.project, lexical, category, context);
      const score = roundConfidence(lexical * 0.25 + category * 0.25 + recency * 0.15 + context * 0.1 + needBoost);
    ...
    .filter((suggestion) => !input.rejectedKeys.has(buildCandidateSuppressionKey(suggestion)))
...
function formatRecommendationBullets(recommendations: ProjectRecommendation[], emptyLine: string): string[] {
...
function freshnessMomentumScore(project: IntelligenceProjectRecord): number {
  return clamp(
    optionScore(project.evidenceFreshness, { Fresh: 55, Aging: 35, Stale: 10 }) +
      optionScore(project.momentum, { Hot: 25, Warm: 15, Cold: 5 }) +
      boolScore(Boolean(project.lastActive), 20),
  );
}

function totalSupportLinks(project: IntelligenceProjectRecord): number {
  return project.relatedResearchIds.length + project.supportingSkillIds.length + project.toolStackIds.length;
}
...
function contextualBoost(project: IntelligenceProjectRecord, tokens: string[]): number {
  const cues = tokenize([...project.projectShape, ...project.deploymentSurface, project.primaryTool]);
  return tokenOverlap(cues, tokens);
}

function recencyBoost(record: Partial<ResearchLibraryRecord & SkillLibraryRecord & ToolMatrixRecord>): number {
  const dates = [record.lastVerified, record.lastPracticed, record.lastReviewed, record.dateResearched].filter(Boolean);
  if (dates.length === 0) {
    return 0.2;
  }
...
function buildMatchReasons(lexical: number, category: number, recency: number, context: number): string[] {
...
function suggestionNeedBoost(
  ...
  const laneBoost =
    project.recommendationLane === "Resume" || project.recommendationLane === "Finish" || project.recommendationLane === "Investigate"
      ? 0.2
      : 0.08;
...
function buildCandidateSuppressionKey(suggestion: CandidateLinkSuggestion): string {
  return `${suggestion.suggestionType}:${suggestion.projectId}:${suggestion.targetId}`;
}

function parseStrategy(raw: unknown): LocalPortfolioIntelligenceViewPlan["strategy"] {
  if (!raw || typeof raw !== "object") {
    throw new AppError("Local portfolio intelligence views config is missing strategy");
  }
  const strategy = raw as Record<string, unknown>;
  if (strategy.primary !== "notion_mcp") {
    throw new AppError('Local portfolio intelligence views strategy.primary must be "notion_mcp"');
  }
  if (strategy.fallback !== "playwright") {
    throw new AppError('Local portfolio intelligence views strategy.fallback must be "playwright"');
  }
  if (!Array.isArray(strategy.notes) || strategy.notes.some((entry) => typeof entry !== "string")) {
    throw new AppError("Local portfolio intelligence views strategy.notes must be a string array");
  }
  return {
    primary: "notion_mcp",
    fallback: "playwright",
...
function parseCollections(raw: unknown): LocalPortfolioIntelligenceViewCollection[] {
  if (!Array.isArray(raw) || raw.length === 0) {
    throw new AppError("Local portfolio intelligence views config must include collections");
  }
  ...
  return raw.map((entry, index) => {
    ...
    if (key !== "projects" && key !== "recommendationRuns" && key !== "linkSuggestions") {
      throw new AppError(`collections[${index}].key must be projects, recommendationRuns, or linkSuggestions`);
    }

    return {
      key,
      database: parseExecutionDataSource(value.database, `collections[${index}].database`),
      views: parseViews(value.views, `collections[${index}].views`),
    };
  });
}

function parseViews(raw: unknown, fieldName: string): LocalPortfolioIntelligenceViewSpec[] {
  if (!Array.isArray(raw) || raw.length === 0) {
...
function optionScore(value: string | undefined, map: Record<string, number>): number {
  return map[value ?? ""] ?? 0;
}

function inverseOptionScore(value: string | undefined, map: Record<string, number>): number {
  return map[value ?? ""] ?? 0;
}
