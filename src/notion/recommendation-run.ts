...
async function main(): Promise<void> {
  try {
    ...
    const [projectSchema, buildSchema, weeklySchema, researchSchema, skillSchema, toolSchema, decisionSchema, packetSchema, taskSchema, runSchema, suggestionSchema, sourceSchema, eventSchema] =
      await Promise.all([
    ...
    const [projectPages, buildPages, weeklyPages, researchPages, skillPages, toolPages, decisionPages, packetPages, taskPages, runPages, suggestionPages, sourcePages, eventPages] =
      await Promise.all([
        ...
        config.phase5ExternalSignals && eventSchema
          ? fetchAllPages(sdk, config.phase5ExternalSignals.events.dataSourceId, eventSchema.titlePropertyName)
          : Promise.resolve([]),
      ]);
    ...
    const externalSources = sourcePages.map((page) => toExternalSignalSourceRecord(page));
    const externalEvents = eventPages.map((page) => toExternalSignalEventRecord(page));
    ...
    const topResume = recommendations.filter((entry) => entry.lane === "Resume")[0];
    const topFinish = recommendations.filter((entry) => entry.lane === "Finish")[0];
    const topInvestigate = recommendations.filter((entry) => entry.lane === "Investigate")[0];
    ...
    if (flags.live) {
      ...
      const created = await api.createPageWithMarkdown({
        ...
        properties: {
          [runSchema.titlePropertyName]: { title: [{ type: "text", text: { content: runTitle } }] },
        },
        markdown,
      });
      createdRun = created;
      await api.updatePageProperties({
        ...
        properties: {
          ...
          "Top Finish Project": relationValue(topFinish ? [topFinish.projectId] : []),
          "Top Investigate Project": relationValue(topInvestigate ? [topInvestigate.projectId] : []),
          "Top Defer Project": relationValue(topDefer ? [topDefer.projectId] : []),
          "Projects Mentioned": relationValue(
            recommendations.slice(0, 8).map((recommendation) => recommendation.projectId),
          ),
      ...
      if (weeklyReview) {
        ...
        const followedRecommendations = packets
          ...
          .map((packet) => `Execution is currently focused on ${resolveProjectTitle(projects, packet.localProjectIds[0] ?? "")}.`);
        const nextMarkdown = mergeManagedSection(
          ...
          renderWeeklyIntelligenceSection({
            ...
            latestRun: {
              ...
              topInvestigateProjectIds: topInvestigate ? [topInvestigate.projectId] : [],
              ...
              reviewedOn: "",
              summary: "",
              referencedProjectIds: recommendations.slice(0, 8).map((recommendation) => recommendation.projectId),
            },
            recommendations: recommendations.slice(0, 5),
            acceptedSuggestions: suggestions.filter((suggestion) => suggestion.status === "Accepted").slice(0, 5),
            rejectedSuggestions: suggestions.filter((suggestion) => suggestion.status === "Rejected").slice(0, 5),
...
function buildDailyFocus(input: {
  ...
  const nowPacket = input.packets.find((packet) => packet.priority === "Now" && packet.status === "In Progress");
  if (nowPacket) {
    const packetTasks = input.tasks
      .filter((task) => task.workPacketIds.includes(nowPacket.id) && task.status !== "Done" && task.status !== "Canceled")
      .slice(0, 3)
  ...
  if (input.topResume) {
    return [`Resume ${input.topResume.projectTitle} as the default daily focus.`];
  }

  const fallbackProject = input.projects.find((project) => project.operatingQueue === "Resume Now");
  return fallbackProject ? [`Resume ${fallbackProject.title} as the fallback daily focus.`] : [];
}
