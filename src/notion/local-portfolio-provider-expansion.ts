...
export function buildProviderExpansionAuditSummary(input: {
	...
	const githubBaselineTrusted =
		input.controlConfig.phase8GithubDeepening?.webhookFeedback.githubStatus ===
		"trusted_feedback";
	...
	const candidateProviders = providers
		.filter(
			(provider) =>
				...
				(provider.policyCount > 0 ||
					provider.externalSignalsEnabled ||
	...
	return {
		...
		nextStep:
			providers.find(
				(provider) =>
					provider.provider !== "GitHub" && provider.readiness !== "blocked",
			)?.nextStep ??
...
function summarizeProviderExpansion(input: {
	...
	const policies = input.policyConfig.policies.filter(
		(policy) => policy.provider === input.provider.name,
	);
	const modeledActionKeys = policies.map((policy) => policy.actionKey).sort();
	const runnerSupportedActionKeys = modeledActionKeys.filter((actionKey) =>
		isActionKeyRunnerSupported(actionKey),
	);
	...
	const externalProvider = input.externalProviderConfig.providers.find(
		(provider) =>
			normalizeExternalProviderKey(provider.key) === input.provider.key,
	);
	const webhookProvider = input.webhookConfig.providers.find(
		(provider) => provider.key === input.provider.key,
	);
	const targetCount = input.targetConfig.targets.filter(
		(target) => inferActuationTargetProvider(target) === input.provider.name,
	).length;
	const missingProviderAuth =
		Boolean(externalProvider?.authEnvVar) &&
		!process.env[externalProvider!.authEnvVar]?.trim();
	...
	if (modeledActionKeys.length > 0 && runnerSupportedActionKeys.length === 0) {
		blockers.push(
			"No action-family or runner support exists for this provider yet.",
		);
	}
	...
	if (
		runnerSupportedActionKeys.length > 0 &&
		modeledActionKeys.length > 0 &&
		targetCount === 0
	) {
	...
	return {
		...
		governanceStatus: readGovernanceStatus(
			input.controlConfig,
			input.provider.key,
		),
		targetCount,
...
export function inferActuationTargetProvider(
	target: ActuationTargetRule,
): GovernanceProviderName | undefined {
	if (
		target.allowedActions.some((actionKey) => actionKey.startsWith("github."))
	) {
		return "GitHub";
	}
	if (
		target.allowedActions.some((actionKey) => actionKey.startsWith("vercel."))
	) {
		return "Vercel";
	}
	...
	if (
		target.sourceUrl?.includes("vercel.com") ||
		target.sourceUrl?.includes(".vercel.app")
	) {
		return "Vercel";
	}
	return undefined;
...
function isActionKeyRunnerSupported(actionKey: string): boolean {
	return RUNNER_SUPPORTED_ACTION_KEYS.includes(
		actionKey as (typeof RUNNER_SUPPORTED_ACTION_KEYS)[number],
	);
}
