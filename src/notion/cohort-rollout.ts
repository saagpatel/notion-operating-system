...
export const DEFAULT_COHORT_PROJECT_ORDER = [
	"BattleGrid",
	"EarthPulse",
	"Relay",
	"SynthWave",
...
export async function runCohortRolloutCommand(
	...
	const token = resolveRequiredNotionToken(
		"NOTION_TOKEN is required for cohort rollout",
	);
	...
	if (
		...
		throw new AppError(
			"Cohort rollout requires phases 2, 5, and 6 to be configured",
		);
	}
	...
	const [projectPages, sourcePages, actionRequestPages, policyPages] =
		await Promise.all([
			fetchAllPages(
				sdk,
			...
			fetchAllPages(
				...
				actionRequestSchema.titlePropertyName,
			),
			fetchAllPages(
				sdk,
	...
	const projectContexts = projectPages.map((page) => ({
		page,
		record: toControlTowerProjectRecord(page),
	}));
	const sourceContexts = sourcePages.map((page) => ({
		page,
		record: toExternalSignalSourceRecord(page),
	}));
	...
	const actionRequests = actionRequestPages.map((page) =>
		toActionRequestRecord(page),
	);
	const projectContextByTitle = new Map(
		projectContexts.map((entry) => [entry.record.title, entry]),
	);
	const sourceContextByProjectId = new Map(
		sourceContexts
			.filter(
				(entry) =>
					entry.record.provider === "GitHub" &&
					entry.record.sourceType === "Repo",
			)
			.flatMap((entry) =>
				entry.record.localProjectIds.map(
					(projectId) => [projectId, entry] as const,
				),
			),
	);
	...
	for (const projectPlan of plan.projects) {
		...
		if (!projectContext) {
			throw new AppError(
				`Could not find project page for "${projectPlan.title}"`,
			);
		}
		...
		const decision = await upsertPageByTitle({
			...
			properties: {
				[decisionSchema.titlePropertyName]: titleValue(
					projectPlan.decisionTitle,
				),
		...
		const request = await ensureGitHubCreateIssueActionRequest({
			...
			approvalReasonApproved:
				"Approved cohort rollout request so this project can move into the governed GitHub issue lane.",
			approvalReasonPending:
				"Pending operator approval for the cohort rollout GitHub issue request.",
			executionNotes:
				"Created by the cohort rollout workflow to move this project from Notion review into a governed GitHub issue.",
			markdownPurpose:
				"Create the governed GitHub issue for this cohort rollout project and use GitHub as the active delivery surface.",
		});
	...
	if (flags.runLive) {
		...
		const liveFollowUpSteps = await runRolloutCommandSteps([
			{
				key: "actionRequestSyncAfterLive",
				script: "portfolio-audit:action-request-sync",
				args: ["--live"],
			},
			{
	...
	const finalFollowUpSteps = await runRolloutCommandSteps([
		{
			key: "controlTowerSync",
			script: "portfolio-audit:control-tower-sync",
			args: ["--live"],
		},
		{
			key: "reviewPacket",
			script: "portfolio-audit:review-packet",
			args: ["--live"],
		},
	]);
	...
	recordCommandOutputSummary(
		...
		},
		{
			...
			warningCategories:
				execution.failures.length > 0 ? ["partial_success"] : undefined,
			metadata: {
				selectedProjects: plan.orderedTitles.length,
			},
		},
...
export function parseCohortProjectSelection(
	raw?: string,
): CohortProjectTitle[] {
	if (!raw?.trim()) {
	...
	const invalid = requested.filter(
		(entry): entry is string =>
			!DEFAULT_COHORT_PROJECT_ORDER.includes(entry as CohortProjectTitle),
	);
	if (invalid.length > 0) {
...
export function buildCohortIssueNextMove(existingNextMove: string): string {
	const focus =
		existingNextMove.trim() ||
		"Define the first delivery slice and capture it in GitHub.";
...
export function buildCohortRolloutPlan(input: {
	...
	const projectByTitle = new Map(
		input.projects.map((project) => [project.title, project]),
	);
	const projects = input.selectedTitles.map((title) => {
		...
		const target = input.targetConfig.targets.find(
			(entry) => entry.title === title || entry.localProjectId === project.id,
		);
		...
		const existingSource = input.githubSources.find(
			(source) =>
				...
				(source.identifier === target.sourceIdentifier ||
					source.sourceUrl === target.sourceUrl),
		);
		const manualSeed =
		...
		const classificationSources = [
			...input.githubSources.filter(
				(source) => !source.localProjectIds.includes(project.id),
			),
			projectedSource,
		];
		...
		if (!candidate.decisionTitle) {
			throw new AppError(
				`Cohort project "${title}" did not produce a GitHub rollout decision`,
			);
		}
	...
	return {
		...
		summary: {
			...
			sourceUpserts: projects.filter(
				(project) => project.sourceAction === "upsert_active",
			).length,
...
function applyCohortProjectAlignment(
...
async function ensureActiveGitHubSource(input: {
	...
	const target = input.targetConfig.targets.find(
		(entry) =>
			entry.title === input.projectTitle ||
			entry.localProjectId === input.projectId,
	);
	if (!target?.sourceIdentifier || !target.sourceUrl) {
		throw new AppError(
			`Missing GitHub actuation target for "${input.projectTitle}"`,
		);
	}
...
function toDecisionCandidate(
	projectPlan: CohortRolloutProjectPlan,
