...
export const DEFAULT_LOCAL_PORTFOLIO_VIEWS_PATH =
	"./config/local-portfolio-views.json";
...
const STRING_FILTERABLE_TYPES = new Set([
	...
	"email",
	"phone_number",
	"formula", // formula fields that return a string (e.g. Operating Queue) are filterable
]);
...
export function parseLocalPortfolioViewPlan(
	raw: unknown,
): LocalPortfolioViewPlan {
	...
	if (version !== 1) {
		throw new AppError(
			`Unsupported local portfolio views config version "${String(version)}"`,
		);
	}
...
export function renderLocalPortfolioViewPlanSummary(
...
export function validateLocalPortfolioViewPlanAgainstSchema(
	...
	if (
		normalizeNotionId(plan.database.dataSourceId) !==
		normalizeNotionId(schema.id)
	) {
		throw new AppError(
	...
	const validatedViews = plan.views.map((view) =>
...
function parseStrategy(raw: unknown): LocalPortfolioViewPlan["strategy"] {
	...
	if (strategy.primary !== "notion_mcp") {
		throw new AppError(
			'Local portfolio views strategy.primary must be "notion_mcp"',
		);
	}
	if (strategy.fallback !== "playwright") {
		throw new AppError(
			'Local portfolio views strategy.fallback must be "playwright"',
		);
	}
	if (
		!Array.isArray(strategy.notes) ||
		strategy.notes.some((note) => typeof note !== "string")
	) {
		throw new AppError(
			"Local portfolio views strategy.notes must be a string array",
		);
	}
...
function parseDatabase(raw: unknown): LocalPortfolioViewPlan["database"] {
	...
	const dataSourceId = requiredString(
...
function parseViews(raw: unknown): LocalPortfolioViewSpec[] {
	if (!Array.isArray(raw) || raw.length === 0) {
		throw new AppError(
			"Local portfolio views config must include at least one view",
		);
	}
...
function validateViewAgainstSchema(
	...
	for (const statement of splitStatements(view.configure)) {
		if (statement.startsWith("SHOW ")) {
			...
			if (properties.length === 0) {
				throw new AppError(
					`View "${view.name}" has a SHOW statement with no properties`,
				);
			}
		...
		if (statement.startsWith("SORT BY ")) {
			...
			const propertyName = getRequiredMatchGroup(
				match,
				1,
				view.name,
				statement,
		...
		if (statement.startsWith("GROUP BY ")) {
			...
			if (!match) {
				throw new AppError(
					`View "${view.name}" has an unsupported GROUP BY statement: ${statement}`,
				);
			}
			const propertyName = getRequiredMatchGroup(
				match,
	...
	if (view.type === "board" && !sawGroupBy) {
		throw new AppError(
			`Board view "${view.name}" must include a GROUP BY statement`,
		);
	}
...
function getRequiredMatchGroup(
	...
	if (!value) {
		throw new AppError(
			`View "${viewName}" has an unsupported configure statement: ${statement}`,
		);
	}
	return value;
}
