...
export interface NotionPageProperty {
	...
	number?: number | null;
	rollup?: { type: string; number?: number | null; function?: string } | null;
	url?: string | null;
}
...
export function toControlTowerProjectRecord(
	...
	return {
		...
		) as ControlTowerProjectRecord["operatingQueue"],
...
export function toBuildSessionRecord(
	page: DataSourcePageRef,
): ControlTowerBuildSessionRecord {
	return {
		id: page.id,
...
export async function upsertPageByTitle(options: {
	...
	const nonTitleProperties = Object.fromEntries(
		Object.entries(options.properties).filter(
			([name]) => name !== options.titlePropertyName,
		),
	);
...
export function selectPropertyValue(value: string | undefined): {
	select?: { name: string };
} {
	return value ? { select: { name: value } } : {};
}

export function datePropertyValue(value: string | undefined): {
	date?: { start: string };
} {
	return value ? { date: { start: value } } : {};
}

export function relationValue(ids: string[]): {
	relation: Array<{ id: string }>;
} {
	return {
