...
interface GitHubRepo {
  ...
  repositoryTopics?: Array<string | { topic?: { name?: string | null } | null }> | null;
  isArchived: boolean;
  isFork: boolean;
  isPrivate: boolean;
  url: string;
...
async function main(): Promise<void> {
  ...
  const plansToTouch = plans.filter((plan) => plan.needsLocalProject || plan.needsSourceRow);
  const createdProjects: Array<{ repo: string; title: string; id: string; url: string }> = [];
  const createdSources: Array<{ repo: string; title: string; id: string; url: string; existed: boolean }> = [];
  ...
  if (flags.live) {
    for (const plan of plansToTouch) {
      ...
      if (!projectPage) {
        throw new AppError(`Could not resolve local project page for ${plan.repo.name}`);
      }
    ...
    if (manualSeedSummaries.length > 0) {
      await syncManualSeeds(sourceConfig, flags.sourceConfig, flags.owner, repos, manualSeedSummaries);
    }
  }
  ...
  const output = {
    ...
    missingLocalProjectCount: plans.filter((plan) => plan.needsLocalProject).length,
...
function buildRepoPlans(input: {
  ...
  return input.repos.map((repo) => {
    ...
    const projectCandidates = buildProjectCandidates(repo.name, metadata.displayName);
    const localProject = findFirstMatch(input.localProjects, projectCandidates);
    const intakeProject = localProject ? undefined : findFirstMatch(input.intakeProjects, projectCandidates);
    const sourceRow = findSourceRow(input.sourceRows, input.owner, repo.name, metadata.displayName);
...
function findFirstMatch(pages: DataSourcePageRef[], candidates: string[]): DataSourcePageRef | undefined {
  const normalizedCandidates = new Set(candidates.map(normalizeKey));
  return pages.find((page) => normalizedCandidates.has(normalizeKey(page.title)));
}
...
function inferProjectMetadata(repo: GitHubRepo, today: string): ProjectMetadata {
  ...
  const haystack = `${repo.name} ${displayName} ${description} ${topics.join(" ")}`.toLowerCase();
  ...
  const buildMaturity = ageDays <= 2 && !repo.description?.trim() ? "Scaffolded" : ageDays <= 2 ? "Scaffolded" : "Functional Core";
  const shipReadiness = buildMaturity === "Scaffolded" ? "Not Ready" : "Needs Hardening";
  const cleanedDescription = stripTrailingPeriod(description);
  const oneLinePitch = cleanedDescription.charAt(0).toUpperCase() + cleanedDescription.slice(1);
  const valueOutcome =
  ...
  const keyIntegrations = inferKeyIntegrations(description, primaryLanguage, topics, deploymentSurface);
  const integrationTags = inferIntegrationTags(`${description} ${topics.join(" ")} ${primaryLanguage}`);
...
function inferCategory(haystack: string): string {
  if (/\bincident\b|\bticket\b|\bworkflow\b|\bsupport\b|\bnetwork\b|\bops\b|\bnotion\b|\bcontrol plane\b/.test(haystack)) {
    return "IT Tool";
  }
  if (/\bdebate\b|\bcritique\b|\bsentiment\b|\bheatmap\b|\bconviction\b|\bneural\b/.test(haystack)) {
...
function inferDeploymentSurface(haystack: string, primaryLanguage: string): string[] {
  const values = new Set<string>();
  if (/\bdesktop\b|\bmacos\b|\bmenu bar\b/.test(haystack)) {
    values.add("Desktop");
  }
  if (/\bweb\b|\bdashboard\b|\bin browser\b|\bnextjs\b|\breact\b/.test(haystack)) {
    values.add("Web");
  }
...
async function createLocalProject(input: {
  ...
  const markdown = renderProjectMarkdown(input.repo, input.owner, input.metadata);
  const result = await upsertPageByTitle({
    ...
    properties: {
      ...
      Momentum: selectPropertyValue(input.metadata.momentum),
      ...
      "Evidence Confidence": selectPropertyValue(input.metadata.evidenceConfidence),
      ...
      "Registry Status": selectPropertyValue(input.metadata.registryStatus),
      ...
      "Monetization / Strategic Value": richTextValue(input.metadata.monetizationValue),
      "Integration Tags": multiSelectValue(input.metadata.integrationTags),
      Summary: richTextValue(`GitHub catch-up row created for ${input.owner}/${input.repo.name}.`),
    },
    markdown,
  });
...
async function upsertOrRepairSource(input: {
  ...
  const markdown = renderSourceMarkdown(input.sourceTitle, input.repo.url, identifier);
  const properties = {
...
async function syncManualSeeds(
  ...
  for (const summary of summaries) {
    ...
    const existingIndex = nextSeeds.findIndex((seed) => seed.identifier === summary.identifier);
...
async function listGitHubRepos(owner: string, limit?: number): Promise<GitHubRepo[]> {
  ...
  const args = ["repo", "list", owner, "--limit", String(limit ?? 200), "--json", jsonFields];
...
function renderProjectMarkdown(repo: GitHubRepo, owner: string, metadata: ProjectMetadata): string {
  return [
...
function renderSourceMarkdown(title: string, sourceUrl: string, identifier: string): string {
  return [
    `# ${title}`,
    "",
    "- Provider: GitHub",
...
function normalizeTopics(
  values: GitHubRepo["repositoryTopics"],
): string[] {
  return (values ?? [])
    .map((value) => {
