...
import { buildProjectIntelligenceDataset, type ProjectIntelligenceRow } from "../portfolio-audit/project-intelligence.js";
import { AppError, toErrorMessage } from "../utils/errors.js";
import { DirectNotionClient } from "./direct-notion-client.js";
import { loadLocalPortfolioControlTowerConfig, type ControlTowerBuildSessionRecord } from "./local-portfolio-control-tower.js";
import {
  datePropertyValue,
  fetchAllPages,
  multiSelectValue,
...
import { buildProjectIntelligenceContext, buildRecommendation, type ProjectRecommendation } from "./local-portfolio-intelligence.js";
import {
...
import { buildExternalSignalSummary, type ExternalSignalEventRecord, type ExternalSignalSourceRecord, type ExternalSignalSummary } from "./local-portfolio-external-signals.js";
import { toExternalSignalEventRecord, toExternalSignalSourceRecord } from "./local-portfolio-external-signals-live.js";
import { toExecutionTaskRecord, toProjectDecisionRecord, toWorkPacketRecord } from "./local-portfolio-execution-live.js";
...
interface LiveProjectSnapshot {
  ...
  buildSessionCount: number | null;
  relatedResearchCount: number | null;
  supportingSkillsCount: number | null;
  linkedToolCount: number | null;
}
...
async function main(): Promise<void> {
  ...
  const token = resolveRequiredNotionToken("NOTION_TOKEN is required to fill empty local project fields");
  ...
  const [
    ...
    config.phase5ExternalSignals
      ? fetchAllPages(sdk, config.phase5ExternalSignals.sources.dataSourceId, "Title")
      : Promise.resolve([]),
    config.phase5ExternalSignals
      ? fetchAllPages(sdk, config.phase5ExternalSignals.events.dataSourceId, "Title")
      : Promise.resolve([]),
  ]);

  const intelligenceByKey = new Map(
    intelligenceProjects.map((project) => [normalizeKey(project.projectName), project] as const),
  );
  ...
  for (const page of pages) {
    ...
    const plan = buildFieldPlan(page, live, intelligence, flags.today, recommendation, summary, latestBuild);
    if (!plan) {
      continue;
    }
  ...
  const output = {
    ...
    fieldCounts: Object.fromEntries([...fieldCounts.entries()].sort((left, right) => right[1] - left[1])),
    failures,
...
function buildFieldPlan(
  ...
  fillRichText(properties, updatedFields, "Primary Run Command", live.primaryRunCommand, firstNonEmpty(intelligence?.primaryRunCommand, deriveRunCommand(live)));
  fillRichText(properties, updatedFields, "Primary Context Doc", live.primaryContextDoc, firstNonEmpty(intelligence?.primaryContextDoc, derivePrimaryContextDoc(live, intelligence)));
  fillRichText(properties, updatedFields, "Summary", live.summary, firstNonEmpty(intelligence?.canonicalSummary, deriveOneLinePitch(live, intelligence)));
  fillRichText(properties, updatedFields, "One-Line Pitch", live.oneLinePitch, deriveOneLinePitch(live, intelligence));
  fillRichText(properties, updatedFields, "Problem Solved", live.problemSolved, firstNonEmpty(intelligence?.problemSolved, live.oneLinePitch, deriveOneLinePitch(live, intelligence)));
  fillRichText(properties, updatedFields, "Primary User", live.primaryUser, derivePrimaryUser(live, intelligence));
  fillRichText(properties, updatedFields, "Last Meaningful Work", live.lastMeaningfulWork, deriveLastMeaningfulWork(live, intelligence, today));
  fillRichText(properties, updatedFields, "Known Risks", live.knownRisks, deriveKnownRisks(live, intelligence));
  fillRichText(properties, updatedFields, "Audit Notes", live.auditNotes, deriveAuditNotes(live, intelligence));
  fillRichText(properties, updatedFields, "Local Path", live.localPath, intelligence?.relativePath || live.title);
  fillRichText(properties, updatedFields, "Start Here", live.startHere, deriveStartHere(live, intelligence));
  fillRichText(properties, updatedFields, "Stack", live.stack, firstNonEmpty(intelligence?.stack, deriveStack(live)));
  fillRichText(properties, updatedFields, "Next Move", live.nextMove, firstNonEmpty(intelligence?.nextMove, deriveNextMove(live)));
  fillRichText(properties, updatedFields, "Biggest Blocker", live.biggestBlocker, intelligence?.biggestBlocker || deriveBlocker(live));
  fillRichText(
    properties,
    updatedFields,
  ...
  fillRichText(properties, updatedFields, "Key Integrations", live.keyIntegrations, deriveKeyIntegrations(live, intelligence));
  fillRichText(properties, updatedFields, "Missing Core Pieces", live.missingCorePieces, deriveMissingCorePieces(live, intelligence));
  fillRichText(
    properties,
    updatedFields,
  ...
  fillRichText(properties, updatedFields, "Value / Outcome", live.valueOutcome, deriveValueOutcome(live, intelligence));
  fillRichText(properties, updatedFields, "Completion", live.completion, deriveCompletion(live, intelligence));
  fillRichText(properties, updatedFields, "Readiness", live.readiness, deriveReadiness(live, intelligence));
  fillRichText(properties, updatedFields, "Merged Into", live.mergedInto, deriveMergedInto(live, intelligence));

  fillSelect(properties, updatedFields, "Status", live.status, intelligence?.canonicalStatus || "In Progress");
  fillSelect(
    properties,
    updatedFields,
    "Pipeline Stage",
  ...
  fillSelect(properties, updatedFields, "Verdict", live.verdict, intelligence?.canonicalVerdict || "Worth Building");
  fillSelect(properties, updatedFields, "Source Group", live.sourceGroup, intelligence?.sourceGroup || "Standalone Projects");
  fillSelect(properties, updatedFields, "Current State", live.currentState, intelligence?.currentState || "Active Build");
  fillSelect(properties, updatedFields, "Portfolio Call", live.portfolioCall, intelligence?.portfolioCall || "Build Now");
  fillSelect(properties, updatedFields, "Category", live.category, intelligence?.canonicalCategory || inferCategory(live, intelligence));
  fillSelect(properties, updatedFields, "Momentum", live.momentum, intelligence?.momentum || "Hot");
  fillSelect(properties, updatedFields, "Docs Quality", live.docsQuality, intelligence?.docsQuality || inferDocsQuality(live));
  fillSelect(properties, updatedFields, "Test Posture", live.testPosture, intelligence?.testPosture || "Unknown");
  fillSelect(properties, updatedFields, "Evidence Confidence", live.evidenceConfidence, intelligence?.evidenceConfidence || "Low");
  fillSelect(properties, updatedFields, "Setup Friction", live.setupFriction, intelligence?.setupFriction || "Low");
  fillSelect(properties, updatedFields, "Runs Locally", live.runsLocally, intelligence?.runsLocally || "Unknown");
  fillSelect(properties, updatedFields, "Build Maturity", live.buildMaturity, intelligence?.buildMaturity || "Idea");
  fillSelect(properties, updatedFields, "Ship Readiness", live.shipReadiness, intelligence?.shipReadiness || "Not Ready");
  fillSelect(properties, updatedFields, "Effort to Demo", live.effortToDemo, intelligence?.effortToDemo || "Unknown");
  fillSelect(properties, updatedFields, "Effort to Ship", live.effortToShip, intelligence?.effortToShip || "Unknown");
  fillSelect(properties, updatedFields, "Registry Status", live.registryStatus, deriveRegistryStatus(live, intelligence));
  fillSelect(properties, updatedFields, "Primary Tool", live.primaryTool, intelligence?.primaryTool || "codex");
  fillSelect(properties, updatedFields, "Context Quality", live.contextQuality, intelligence?.contextQuality || "standard");
  fillSelect(properties, updatedFields, "Recommendation Lane", live.recommendationLane, recommendation.lane);
  fillSelect(properties, updatedFields, "Recommendation Confidence", live.recommendationConfidence, recommendation.confidence);
  fillSelect(properties, updatedFields, "External Signal Coverage", live.externalSignalCoverage, summary.coverage);
  fillSelect(properties, updatedFields, "Latest Deployment Status", live.latestDeploymentStatus, summary.latestDeploymentStatus);

  fillMultiSelect(properties, updatedFields, "Project Shape", live.projectShape, firstNonEmptyArray(intelligence?.projectShape, deriveProjectShape(live, intelligence)));
  fillMultiSelect(
    properties,
    updatedFields,
    "Deployment Surface",
    live.deploymentSurface,
    firstNonEmptyArray(intelligence?.deploymentSurface, deriveDeploymentSurface(live, intelligence)),
  );
  fillMultiSelect(properties, updatedFields, "Integration Tags", live.integrationTags, deriveIntegrationTags(live, intelligence));

  fillDate(properties, updatedFields, "Date Updated", live.dateUpdated, today);
  fillDate(properties, updatedFields, "Last Active", live.lastActive, intelligence?.lastActive || today);
  fillDate(properties, updatedFields, "Recommendation Updated", live.recommendationUpdated, today);
  fillDate(
    properties,
    updatedFields,
  ...
  fillRichText(properties, updatedFields, "Last Build Session", live.lastBuildSession, latestBuild.label);

  fillNumber(properties, updatedFields, "Recommendation Score", live.recommendationScore, recommendation.score);
  fillNumber(properties, updatedFields, "Open PR Count", live.openPrCount, summary.openPrCount);
  fillNumber(
    properties,
    updatedFields,
  ...
  fillNumber(properties, updatedFields, "Build Session Count", live.buildSessionCount, 0);
  fillNumber(properties, updatedFields, "Related Research Count", live.relatedResearchCount, 0);
  fillNumber(properties, updatedFields, "Supporting Skills Count", live.supportingSkillsCount, 0);
  fillNumber(properties, updatedFields, "Linked Tool Count", live.linkedToolCount, 0);
...
function readLiveProjectSnapshot(page: DataSourcePageRef): LiveProjectSnapshot {
  return {
    ...
    oneLinePitch: textValue(page.properties["One-Line Pitch"]) || textValue(page.properties.Summary),
    ...
    dateUpdated: page.properties["Date Updated"]?.date?.start?.slice(0, 10) ?? "",
    lastActive: page.properties["Last Active"]?.date?.start?.slice(0, 10) ?? "",
    stack: textValue(page.properties.Stack),
    projectShape: (page.properties["Project Shape"]?.multi_select ?? []).map((entry) => entry.name ?? "").filter(Boolean),
    deploymentSurface: (page.properties["Deployment Surface"]?.multi_select ?? []).map((entry) => entry.name ?? "").filter(Boolean),
    ...
    registryStatus: selectValue(page.properties["Registry Status"]),
    ...
    integrationTags: (page.properties["Integration Tags"]?.multi_select ?? []).map((entry) => entry.name ?? "").filter(Boolean),
    lastBuildSession: textValue(page.properties["Last Build Session"]),
    lastBuildSessionDate: page.properties["Last Build Session Date"]?.date?.start?.slice(0, 10) ?? "",
    externalSignalCoverage: selectValue(page.properties["External Signal Coverage"]),
    latestExternalActivity: page.properties["Latest External Activity"]?.date?.start?.slice(0, 10) ?? "",
    latestDeploymentStatus: selectValue(page.properties["Latest Deployment Status"]),
    openPrCount: page.properties["Open PR Count"]?.number ?? null,
    recentFailedWorkflowRuns: page.properties["Recent Failed Workflow Runs"]?.number ?? null,
    externalSignalUpdated: page.properties["External Signal Updated"]?.date?.start?.slice(0, 10) ?? "",
    recommendationLane: selectValue(page.properties["Recommendation Lane"]),
    recommendationScore: page.properties["Recommendation Score"]?.number ?? null,
    recommendationConfidence: selectValue(page.properties["Recommendation Confidence"]),
    recommendationUpdated: page.properties["Recommendation Updated"]?.date?.start?.slice(0, 10) ?? "",
    buildSessionCount: page.properties["Build Session Count"]?.number ?? null,
    relatedResearchCount: page.properties["Related Research Count"]?.number ?? null,
    supportingSkillsCount: page.properties["Supporting Skills Count"]?.number ?? null,
    linkedToolCount: page.properties["Linked Tool Count"]?.number ?? null,
  };
}
...
function deriveRunCommand(live: LiveProjectSnapshot): string {
  if (live.startHere.trim()) {
    return live.startHere.replace(/^Open\s+/i, "").trim() || "Open the local files directly";
  }
  return "Open the local files directly";
}

function derivePrimaryContextDoc(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
...
function deriveOneLinePitch(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
...
function derivePrimaryUser(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
  const text = `${live.category} ${live.oneLinePitch} ${intelligence?.canonicalSummary ?? ""}`.toLowerCase();
...
function deriveLastMeaningfulWork(
  ...
  const state = (live.currentState || intelligence?.currentState || "Active Build").toLowerCase();
  return `The latest meaningful work was captured around ${anchor}, and the project is currently in a ${state} posture.`;
}

function deriveKnownRisks(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
  const blocker = firstNonEmpty(live.biggestBlocker, intelligence?.biggestBlocker);
  if (blocker) {
    return blocker;
  }
...
function deriveProjectHealthNotes(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
  const state = live.currentState || intelligence?.currentState || "Active Build";
  const momentum = live.momentum || intelligence?.momentum || "Warm";
  const build = live.buildMaturity || intelligence?.buildMaturity || "Functional Core";
  const readiness = live.shipReadiness || intelligence?.shipReadiness || "Needs Hardening";
  return `${state} with ${momentum.toLowerCase()} momentum. Build maturity is ${build.toLowerCase()} and ship readiness is ${readiness.toLowerCase()}.`;
}

function deriveAuditNotes(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
  const summary = firstNonEmpty(live.oneLinePitch, intelligence?.canonicalSummary, deriveOneLinePitch(live, intelligence));
  return `Audit refresh captured this project as ${summary.toLowerCase()}`;
}

function deriveKeyIntegrations(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
...
function deriveMissingCorePieces(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
  ...
  const readiness = live.shipReadiness || intelligence?.shipReadiness || "Not Ready";
...
function deriveMonetizationValue(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
  const text = `${live.category} ${live.oneLinePitch} ${intelligence?.canonicalSummary ?? ""}`.toLowerCase();
...
function deriveValueOutcome(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
  if (intelligence?.valueOutcome?.trim()) {
    return intelligence.valueOutcome;
  }
  const summary = firstNonEmpty(live.problemSolved, live.oneLinePitch, intelligence?.canonicalSummary);
  if (!summary) {
...
function deriveCompletion(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
...
function deriveReadiness(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
  if (intelligence?.readiness?.trim()) {
    return intelligence.readiness;
  }
  const build = live.buildMaturity || intelligence?.buildMaturity || "Functional Core";
  const readiness = live.shipReadiness || intelligence?.shipReadiness || "Not Ready";
  const docs = (live.docsQuality || intelligence?.docsQuality || "Usable").toLowerCase();
  const tests = (live.testPosture || intelligence?.testPosture || "unknown").toLowerCase();
  const runStatus = (live.runsLocally || intelligence?.runsLocally || "Unknown").toLowerCase();
...
function deriveMergedInto(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
...
function deriveRegistryStatus(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
  const state = (live.currentState || intelligence?.currentState || "").toLowerCase();
  if (state === "archived") {
    return "archived";
  }
  if (state === "parked") {
...
function deriveStartHere(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
...
function inferCategory(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string {
  const text = `${live.title} ${live.oneLinePitch} ${intelligence?.canonicalSummary ?? ""}`.toLowerCase();
...
function deriveProjectShape(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string[] {
  if (/\bsandbox\b/i.test(live.title)) {
    return ["System", "Tool"];
  }
  const category = live.category || intelligence?.canonicalCategory || inferCategory(live, intelligence);
...
function deriveDeploymentSurface(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string[] {
  if (/\bsandbox\b/i.test(live.title)) {
    return ["Internal Tool"];
  }
  const text = `${live.title} ${live.oneLinePitch} ${live.stack} ${intelligence?.canonicalSummary ?? ""}`.toLowerCase();
  const values: string[] = [];
  if (/desktop|macos|menu bar|clipboard/.test(text)) {
    values.push("Desktop");
  }
  ...
  if (values.length === 0 && (live.localPath.trim() || intelligence?.relativePath?.trim())) {
    values.push("CLI");
  }
  return uniqueStrings(values.length > 0 ? values : ["Web"]);
}

function deriveIntegrationTags(live: LiveProjectSnapshot, intelligence?: ProjectIntelligenceRow): string[] {
  const text = `${deriveKeyIntegrations(live, intelligence)} ${live.stack} ${live.title} ${live.localPath} ${intelligence?.relativePath ?? ""}`.toLowerCase();
  ...
  if (values.size === 0 && (live.localPath.trim() || intelligence?.relativePath?.trim())) {
    values.add("GitHub");
  }
  return [...values];
}

function groupBuildSessionsByProjectId(buildSessions: ControlTowerBuildSessionRecord[]): Map<string, ControlTowerBuildSessionRecord[]> {
  return buildSessions.reduce<Map<string, ControlTowerBuildSessionRecord[]>>((map, session) => {
...
function groupWorkflowRunsByProjectId(events: ExternalSignalEventRecord[]): Map<string, ExternalSignalEventRecord[]> {
  return events
...
function deriveLatestBuildEvidence(input: {
  ...
  return {
    date: input.summary.latestExternalActivity || input.live.lastActive || input.intelligence?.lastActive || input.today,
...
const SPECIAL_STACK_OVERRIDES = new Map<string, string>([
  ["Screenshot to Data Select", "Desktop workflow, screenshot processing, local selection tooling"],
  ["da-scaffold", "Project scaffolding, local templates, developer workflow tooling"],
  ["Sandbox Local Portfolio Project", "Notion workflow sandbox, local portfolio automation, internal tooling"],
]);
