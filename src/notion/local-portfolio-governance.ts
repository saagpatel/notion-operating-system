...
export type ApprovalRule = "No Write" | "Single Approval" | "Dual Approval" | "Emergency";
...
export type DeliveryStatus = "Received" | "Verified" | "Rejected" | "Duplicate" | "Processed" | "Failed";
export type VerificationResult = "Valid" | "Invalid Signature" | "Unknown Endpoint" | "Expired" | "Duplicate";
...
export function ensurePhase6GovernanceState(
  ...
  return {
    policies: config.phase6Governance?.policies ?? blankDatabaseRef("External Action Policies", "external_action_policies"),
    actionRequests:
      config.phase6Governance?.actionRequests ?? blankDatabaseRef("External Action Requests", "external_action_requests"),
    webhookEndpoints:
      config.phase6Governance?.webhookEndpoints ?? blankDatabaseRef("Webhook Endpoints", "webhook_endpoints"),
    webhookDeliveries:
      config.phase6Governance?.webhookDeliveries ?? blankDatabaseRef("Webhook Deliveries", "webhook_deliveries"),
    webhookReceipts:
      config.phase6Governance?.webhookReceipts ?? blankDatabaseRef("Webhook Receipts", "webhook_receipts"),
    ...
    replayAndDedupe: config.phase6Governance?.replayAndDedupe ?? {
      ...
      googleCalendar: { replayWindowMinutes: 240, dedupeKey: "provider+delivery_id" },
    },
    ...
    envRefs: config.phase6Governance?.envRefs ?? {
      ...
      breakGlassEnvVars: ["GITHUB_BREAK_GLASS_TOKEN", "VERCEL_BREAK_GLASS_TOKEN"],
    },
    ...
    baselineCapturedAt: config.phase6Governance?.baselineCapturedAt ?? input.today,
...
export function buildGovernanceAuditSummary(input: {
  ...
  const endpointModeWarnings = input.providerConfig.providers.flatMap((provider) => {
    ...
    if (provider.mode === "shadow" && configuredStatus === "live") {
      return [`${provider.displayName} is marked live in config while the provider plan is still shadow-only.`];
    }
    return [];
  });
  ...
  const identityWarnings = input.policyConfig.policies.flatMap((policy) => {
    if (policy.provider === "GitHub" && policy.identityType !== "GitHub App") {
      return [`${policy.actionKey} is not using the app-first GitHub identity posture.`];
    }
    return [];
  });
...
export function renderGovernanceBriefSection(input: {
  ...
  const latestDeliveries = [...input.deliveries]
    ...
    .slice(0, 5);
  const githubPolicies = input.policies.filter((policy) => policy.provider === "GitHub");
  ...
  return [
    ...
    ...(openRequests.length > 0
      ? openRequests.map((request) => `- [${request.title}](${request.url}) - ${request.status}${request.expiresAt ? ` / expires ${request.expiresAt}` : ""}`)
      : ["- No pending governance requests for this project right now."]),
    "",
    "### Latest Deliveries",
    ...(latestDeliveries.length > 0
      ? latestDeliveries.map((delivery) => `- [${delivery.title}](${delivery.url}) - ${delivery.status} / ${delivery.verificationResult} / ${delivery.lastSeenAt}`)
      : ["- No webhook deliveries linked to this project yet."]),
    "",
    "### Recent Actuation",
    ...(recentExecutions.length > 0
      ? recentExecutions.map((execution) => `- [${execution.title}](${execution.url}) - ${execution.mode} / ${execution.status} / ${execution.executedAt}`)
...
export function renderGovernanceCommandCenterSection(input: {
  ...
  const duplicateDeliveries = input.deliveries.filter((delivery) => delivery.status === "Duplicate");
  const rejectedDeliveries = input.deliveries.filter((delivery) => delivery.verificationResult !== "Valid");
  const shadowReadyEndpoints = input.endpoints.filter((endpoint) => endpoint.mode === "Shadow");
  const disabledPolicies = input.policies.filter((policy) => policy.executionMode === "Disabled");
  const recentActuationFailures = [...(input.actuationExecutions ?? [])]
    .filter((execution) => execution.status === "Failed" || execution.status === "Compensation Needed")
    .sort((left, right) => right.executedAt.localeCompare(left.executedAt))
    .slice(0, 8);
  ...
  return [
    ...
    ...(pendingRequests.length > 0
      ? pendingRequests.slice(0, 8).map((request) => `- [${request.title}](${request.url})`)
      : ["- None right now."]),
    "",
    "### Recent Actuation Failures",
    ...(recentActuationFailures.length > 0
      ? recentActuationFailures.map((execution) => `- [${execution.title}](${execution.url}) - ${execution.status}`)
    ...
    ...(duplicateDeliveries.length + rejectedDeliveries.length > 0
      ? [...duplicateDeliveries, ...rejectedDeliveries]
          .slice(0, 8)
          .map((delivery) => `- [${delivery.title}](${delivery.url}) - ${delivery.status} / ${delivery.verificationResult}`)
...
export function renderWeeklyGovernanceSection(input: {
  ...
  const verified = input.deliveries.filter((delivery) => delivery.verificationResult === "Valid").length;
  const rejected = input.deliveries.filter((delivery) => delivery.verificationResult !== "Valid").length;
  const liveActions = (input.actuationExecutions ?? []).filter(
    (execution) => execution.mode === "Live" && execution.status === "Succeeded",
  ).length;
  const failedActions = (input.actuationExecutions ?? []).filter((execution) => execution.status === "Failed").length;
...
export function validateLocalPortfolioGovernanceViewPlanAgainstSchemas(input: {
  ...
  for (const collection of input.plan.collections) {
    ...
    if (!schema) {
      throw new AppError(`Missing schema for governance view collection "${collection.key}"`);
    }
...
export function verifyGitHubSignature(secret: string, payload: string | Buffer, header: string): boolean {
...
export function verifyVercelSignature(secret: string, payload: string | Buffer, header: string): boolean {
...
export function createWebhookReceiptEnvelope(input: {
  ...
  const deliveryId = extractDeliveryId(input.providerPlan, headers, parsedPayload);
  const eventType = extractEventType(input.providerPlan, headers, parsedPayload);
  const signatureHeader = headers[input.providerPlan.signatureHeader.toLowerCase()] ?? "";
  ...
  const verificationResult: VerificationResult = valid ? "Valid" : "Invalid Signature";
  const logicalDeliveryKey = buildLogicalDeliveryKey(normalizeProviderKey(input.providerPlan.displayName), deliveryId);
  ...
  return {
    ...
    headers: Object.fromEntries(Object.entries(headers).filter(([key]) => ALLOWED_HEADER_EXCERPT.has(key))),
    body: input.body,
    ...
    logicalDeliveryKey,
    eventType,
    eventKey: normalized?.eventKey ?? buildLogicalDeliveryKey(normalizeProviderKey(input.providerPlan.displayName), deliveryId),
    status: verificationResult === "Valid" ? "Verified" : "Rejected",
    failureNotes: verificationResult === "Valid" ? "" : `Signature validation failed for ${input.providerPlan.displayName}.`,
  };
}
...
export function buildNormalizedWebhookEvent(input: {
  ...
  if (input.provider === "github") {
    ...
    const repoUrl =
      typeof repository?.html_url === "string" ? repository.html_url.trim() : typeof repository?.url === "string" ? repository.url.trim() : "";
    ...
    if (input.eventType === "pull_request") {
      const number = readNestedNumber(input.payload, ["pull_request", "number"]) ?? readNestedNumber(input.payload, ["number"]);
      const title = readNestedString(input.payload, ["pull_request", "title"]) || `Pull Request ${number ?? ""}`.trim();
      const action = readNestedString(input.payload, ["action"]) || readNestedString(input.payload, ["pull_request", "state"]) || "open";
      return {
        provider: "GitHub",
        signalType: "Pull Request",
        status: action,
        severity: ["closed", "merged"].includes(action.toLowerCase()) ? "Info" : "Watch",
        occurredAt,
        title: number ? `PR #${number} - ${title}` : title,
        sourceIdValue: repoFullName,
        sourceUrl: readNestedString(input.payload, ["pull_request", "html_url"]) || repoUrl,
        environment: "N/A",
        eventKey: buildLogicalDeliveryKey("github", `${repoFullName}::pull_request::${number ?? title}`),
    ...
    if (input.eventType === "workflow_run") {
      const name = readNestedString(input.payload, ["workflow_run", "name"]) || "Workflow Run";
      const status =
      ...
      return {
        ...
        sourceIdValue: repoFullName,
        sourceUrl: readNestedString(input.payload, ["workflow_run", "html_url"]) || repoUrl,
        environment: "N/A",
  ...
  if (input.provider === "vercel") {
    ...
    const deploymentState =
      ...
      "unknown";
    const environment =
      readNestedString(input.payload, ["deployment", "target"]) === "production" ? "Production" : "Preview";
    const occurredAt = firstNonEmptyDate([
    ...
    return {
      ...
      environment,
      eventKey: buildLogicalDeliveryKey("vercel", `${projectId || projectName}::deployment::${deploymentState}`),
...
export function buildLogicalDeliveryKey(provider: GovernanceProviderKey, deliveryId: string): string {
  return `${provider}::${deliveryId.trim().toLowerCase()}`;
}

export function buildRequestExpiryDate(startDate: string, hours: number): string {
  const date = new Date(`${startDate}T00:00:00Z`);
  date.setUTCHours(date.getUTCHours() + hours);
  return date.toISOString().slice(0, 10);
}

export function shouldExpireActionRequest(request: ActionRequestRecord, today: string): boolean {
  return request.status === "Approved" && Boolean(request.expiresAt) && request.expiresAt < today;
}

export function resolveGovernanceSpoolDirectory(config: LocalPortfolioControlTowerConfig): string {
  return path.resolve(requirePhase6Governance(config).receiver.spoolDirectory);
}

export function parseLocalPortfolioGovernancePolicyConfig(raw: unknown): LocalPortfolioGovernancePolicyConfig {
  if (!raw || typeof raw !== "object") {
    throw new AppError("Local portfolio governance policy config must be an object");
  }
  const value = raw as Record<string, unknown>;
  if (value.version !== 1) {
    throw new AppError(`Unsupported local portfolio governance policy config version "${String(value.version)}"`);
  }
  return {
    version: 1,
    strategy: parseDirectRestStrategy(value.strategy, "localPortfolioGovernancePolicies.strategy"),
    policies: parseGovernancePolicies(value.policies),
  };
}

export function parseLocalPortfolioWebhookProviderConfig(raw: unknown): LocalPortfolioWebhookProviderConfig {
  if (!raw || typeof raw !== "object") {
    throw new AppError("Local portfolio webhook provider config must be an object");
  }
  const value = raw as Record<string, unknown>;
  if (value.version !== 1) {
    throw new AppError(`Unsupported local portfolio webhook provider config version "${String(value.version)}"`);
  }
  return {
    version: 1,
    spoolDirectory: requiredString(value.spoolDirectory, "localPortfolioWebhookProviders.spoolDirectory"),
    providers: parseWebhookProviders(value.providers),
  };
}

export function parseLocalPortfolioGovernanceViewPlan(raw: unknown): LocalPortfolioGovernanceViewPlan {
  if (!raw || typeof raw !== "object") {
    throw new AppError("Local portfolio governance views config must be an object");
  }
  const value = raw as Record<string, unknown>;
  if (value.version !== 1) {
    throw new AppError(`Unsupported local portfolio governance views config version "${String(value.version)}"`);
  }
  return {
    version: 1,
...
function parseGovernancePolicies(raw: unknown): GovernancePolicyPlan[] {
  if (!Array.isArray(raw) || raw.length === 0) {
    throw new AppError("localPortfolioGovernancePolicies.policies must include at least one policy");
  }
  return raw.map((entry, index) => {
    if (!entry || typeof entry !== "object") {
      throw new AppError(`localPortfolioGovernancePolicies.policies[${index}] must be an object`);
    }
    const value = entry as Record<string, unknown>;
    return {
      actionKey: requiredString(value.actionKey, `policies[${index}].actionKey`),
      provider: parseProviderName(requiredString(value.provider, `policies[${index}].provider`)),
      mutationClass: parseMutationClass(requiredString(value.mutationClass, `policies[${index}].mutationClass`)),
      executionMode: parseExecutionMode(requiredString(value.executionMode, `policies[${index}].executionMode`)),
      identityType: parseIdentityType(requiredString(value.identityType, `policies[${index}].identityType`)),
      approvalRule: parseApprovalRule(requiredString(value.approvalRule, `policies[${index}].approvalRule`)),
      dryRunRequired: requiredBoolean(value.dryRunRequired, `policies[${index}].dryRunRequired`),
      rollbackRequired: requiredBoolean(value.rollbackRequired, `policies[${index}].rollbackRequired`),
      defaultExpiryHours: requiredPositiveNumber(value.defaultExpiryHours, `policies[${index}].defaultExpiryHours`),
      allowedSources: parseActionSourceTypes(value.allowedSources, `policies[${index}].allowedSources`),
...
function parseWebhookProviders(raw: unknown): GovernanceWebhookProviderPlan[] {
  if (!Array.isArray(raw) || raw.length === 0) {
    throw new AppError("localPortfolioWebhookProviders.providers must include at least one provider");
  }
  return raw.map((entry, index) => {
    if (!entry || typeof entry !== "object") {
      throw new AppError(`localPortfolioWebhookProviders.providers[${index}] must be an object`);
    }
    const value = entry as Record<string, unknown>;
    return {
      key: parseProviderKey(requiredString(value.key, `providers[${index}].key`)),
      displayName: parseProviderName(requiredString(value.displayName, `providers[${index}].displayName`)),
      mode: parseShadowMode(requiredString(value.mode, `providers[${index}].mode`)),
      endpointPath: requiredString(value.endpointPath, `providers[${index}].endpointPath`),
      subscribedEvents: requiredStringArray(value.subscribedEvents, `providers[${index}].subscribedEvents`),
      secretEnvVar: requiredString(value.secretEnvVar, `providers[${index}].secretEnvVar`),
      deliveryIdLocation: parseStringEnum(
        value.deliveryIdLocation,
        `providers[${index}].deliveryIdLocation`,
        ["header", "payload"],
      ),
      deliveryIdField: requiredString(value.deliveryIdField, `providers[${index}].deliveryIdField`),
      eventTypeLocation: parseStringEnum(
        value.eventTypeLocation,
        `providers[${index}].eventTypeLocation`,
        ["header", "payload"],
      ),
      eventTypeField: requiredString(value.eventTypeField, `providers[${index}].eventTypeField`),
      signatureHeader: requiredString(value.signatureHeader, `providers[${index}].signatureHeader`),
...
function parseGovernanceViewCollections(raw: unknown): GovernanceViewCollection[] {
  if (!Array.isArray(raw) || raw.length === 0) {
    throw new AppError("localPortfolioGovernanceViews.collections must include collections");
  }
  return raw.map((entry, index) => {
    if (!entry || typeof entry !== "object") {
      throw new AppError(`localPortfolioGovernanceViews.collections[${index}] must be an object`);
    }
    const value = entry as Record<string, unknown>;
    ...
    return {
      key,
      database: parseGovernanceDatabaseRef(value.database, `collections[${index}].database`),
      views: parseGovernanceViews(value.views, `collections[${index}].views`),
    };
  });
}

function parseGovernanceDatabaseRef(raw: unknown, fieldName: string): GovernanceDatabaseRef {
  if (!raw || typeof raw !== "object") {
...
function parseGovernanceViews(raw: unknown, fieldName: string): GovernanceViewSpec[] {
  if (!Array.isArray(raw) || raw.length === 0) {
  ...
  return raw.map((entry, index) => {
    ...
    const value = entry as Record<string, unknown>;
    const type = parseStringEnum(value.type, `${fieldName}[${index}].type`, ["table", "board", "gallery"]);
    return {
...
function validateViewAgainstSchema(view: GovernanceViewSpec, schema: DataSourceSchemaSnapshot): string[] {
...
function parseViewStrategy(
  ...
  if (!raw || typeof raw !== "object") {
    throw new AppError("localPortfolioGovernanceViews.strategy must be an object");
  }
  const value = raw as Record<string, unknown>;
  if (value.primary !== "notion_mcp" || value.fallback !== "playwright") {
    throw new AppError("localPortfolioGovernanceViews.strategy must be notion_mcp/playwright");
  }
  return {
    primary: "notion_mcp",
    fallback: "playwright",
    notes: requiredStringArray(value.notes, "localPortfolioGovernanceViews.strategy.notes"),
  };
}
...
function parseMutationClass(value: string): MutationClass {
  if (value !== "Read" && value !== "Comment" && value !== "Issue" && value !== "Deployment Control") {
...
function parseIdentityType(value: string): IdentityType {
  if (value !== "GitHub App" && value !== "Team Token" && value !== "Break Glass Token") {
...
function parseApprovalRule(value: string): ApprovalRule {
  if (value !== "No Write" && value !== "Single Approval" && value !== "Dual Approval" && value !== "Emergency") {
    throw new AppError(`Unsupported approval rule "${value}"`);
  }
  return value;
}

function parseActionSourceTypes(value: unknown, fieldName: string): ActionSourceType[] {
  return requiredStringArray(value, fieldName).map((entry) =>
    parseStringEnum(entry, `${fieldName}[]`, ["Recommendation", "Weekly Review", "Manual"]),
  );
}
...
function lowercaseHeaders(input: Record<string, string | undefined>): Record<string, string> {
...
function readNestedString(object: Record<string, unknown>, pathParts: string[]): string {
...
function readNestedNumber(object: Record<string, unknown>, pathParts: string[]): number | undefined {
  let current: unknown = object;
  for (const part of pathParts) {
    if (!current || typeof current !== "object") {
      return undefined;
...
function isFailureLike(value: string): boolean {
  ...
  return ["failed", "failure", "error", "cancelled", "canceled", "timed_out"].includes(normalized);
}
...
function blankDatabaseRef(name: string, destinationAlias: string): GovernanceDatabaseRef {
  return {
    name,
...
function normalizeProviderKey(value: GovernanceProviderName): GovernanceProviderKey {
  switch (value) {
    case "GitHub":
      return "github";
    case "Vercel":
...
function assertPropertyExists(schema: DataSourceSchemaSnapshot, viewName: string, propertyName: string): PropertySchema {
...
function parseStringEnum<T extends string>(value: unknown, fieldName: string, allowed: T[]): T {
...
const FILTERABLE_TYPES = new Set(["title", "rich_text", "select", "status", "checkbox", "date", "number"]);
