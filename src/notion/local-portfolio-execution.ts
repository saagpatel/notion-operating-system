...
export const DEFAULT_LOCAL_PORTFOLIO_EXECUTION_VIEWS_PATH = "./config/local-portfolio-execution-views.json";
...
export function parseLocalPortfolioExecutionViewPlan(raw: unknown): LocalPortfolioExecutionViewPlan {
  if (!raw || typeof raw !== "object") {
    throw new AppError("Local portfolio execution views config must be an object");
  }

  const plan = raw as Record<string, unknown>;
  if (plan.version !== 1) {
    throw new AppError(`Unsupported local portfolio execution views config version "${String(plan.version)}"`);
  }
...
export function calculateExecutionMetrics(input: {
  ...
  const standbyPackets = input.packets.filter(
    (packet) => packet.priority === "Standby" && !isWorkPacketClosed(packet.status),
  );
  const blockedPackets = input.packets.filter((packet) => packet.status === "Blocked");
  const blockedTasks = input.tasks.filter((task) => task.status === "Blocked");
  const overdueTasks = input.tasks.filter(
    (task) => task.dueDate && task.dueDate < input.today && !isExecutionTaskClosed(task.status),
  );
  ...
  const packetsCompletedThisWeek = input.packets.filter(
    (packet) => packet.status === "Done" && packet.targetFinish && diffDays(packet.targetFinish, input.today) <= 7,
  ).length;
  const rolloverPackets = input.packets.filter((packet) => packet.rolloverCount > 0).length;
  ...
  return {
    openDecisions: input.decisions.filter((decision) => decision.status === "Proposed").length,
...
export function validateExecutionWip(input: {
  ...
  const nowPackets = input.packets.filter((packet) => packet.priority === "Now" && !isWorkPacketClosed(packet.status));
  const standbyPackets = input.packets.filter(
    (packet) => packet.priority === "Standby" && !isWorkPacketClosed(packet.status),
  );
  ...
  for (const packet of input.packets) {
    if (!isWorkPacketClosed(packet.status) && packet.localProjectIds.length !== 1) {
      violations.push(`Packet "${packet.title}" must belong to exactly one project.`);
    }
  }

  for (const task of input.tasks) {
    if (!isExecutionTaskClosed(task.status) && task.workPacketIds.length !== 1) {
      violations.push(`Task "${task.title}" must belong to exactly one work packet.`);
    }
  }
...
export function buildProjectExecutionContext(input: {
  ...
  const projectDecisions = input.decisions
    ...
    .sort((left, right) => compareIsoDate(right.proposedOn || right.decidedOn, left.proposedOn || left.decidedOn));
  const projectPackets = input.packets
    .filter((packet) => packet.localProjectIds.includes(projectId))
    .sort((left, right) => compareIsoDate(right.targetStart || right.targetFinish, left.targetStart || left.targetFinish));
  const projectTasks = input.tasks.filter((task) => task.localProjectIds.includes(projectId));

  return {
    project: input.project,
    activePacket: projectPackets.find((packet) => packet.priority === "Now" && !isWorkPacketClosed(packet.status)),
    standbyPacket: projectPackets.find(
      (packet) => packet.priority === "Standby" && !isWorkPacketClosed(packet.status),
    ),
    openDecisions: projectDecisions.filter((decision) => decision.status === "Proposed").slice(0, 5),
    blockedTasks: projectTasks.filter((task) => task.status === "Blocked").slice(0, 5),
...
export function renderExecutionBriefSection(context: ProjectExecutionContext): string {
  const lines = [
    "<!-- codex:notion-execution-brief:start -->",
    "## Execution Brief",
    "",
    `Updated: ${context.project.nextReviewDate || context.project.dateUpdated || "No review date yet"}`,
    "",
    "### Current Packet",
    ...formatPacketBullets(context.activePacket ? [context.activePacket] : [], (packet) => [
    ...
    "",
    "### Standby Packet",
    ...formatPacketBullets(context.standbyPacket ? [context.standbyPacket] : [], (packet) => [
      packet.whyNow || "Why-now note missing",
      packet.targetStart ? `target start ${packet.targetStart}` : "",
      packet.estimatedSize ? `size ${packet.estimatedSize.toLowerCase()}` : "",
    ], "- No standby packet yet."),
    "",
    "### Open Decisions",
    ...formatDecisionBullets(context.openDecisions, (decision) => [
      decision.decisionType ? `type ${decision.decisionType.toLowerCase()}` : "",
      decision.revisitBy ? `revisit ${decision.revisitBy}` : "",
      decision.rationale || decision.expectedImpact || "",
    ], "- No open project-level decisions."),
    "",
    "### Blocked Tasks",
    ...formatTaskBullets(context.blockedTasks, (task) => [
    ...
    "",
    "### Next Due Tasks",
    ...formatTaskBullets(context.dueTasks, (task) => [
    ...
    ...formatBuildSessionBullets(context.recentBuildSessions, "- No recent build sessions linked yet."),
    "",
    "### Packet History",
    ...formatPacketBullets(context.packetHistory, (packet) => [
...
export function renderExecutionCommandCenterSection(input: {
  ...
  const nowPackets = input.packets
    .filter((packet) => packet.priority === "Now" && !isWorkPacketClosed(packet.status))
    .slice(0, 5);
  const standbyPackets = input.packets
    .filter((packet) => packet.priority === "Standby" && !isWorkPacketClosed(packet.status))
    .slice(0, 5);
  const openDecisions = input.decisions.filter((decision) => decision.status === "Proposed").slice(0, 8);
  const blockedTasks = input.tasks.filter((task) => task.status === "Blocked").slice(0, 8);
  const overdueTasks = input.tasks
    .filter((task) => task.dueDate && task.dueDate < input.today && !isExecutionTaskClosed(task.status))
    .sort((left, right) => compareIsoDate(left.dueDate, right.dueDate))
    .slice(0, 8);
  const rolloverRisk = input.packets
    .filter((packet) => !isWorkPacketClosed(packet.status) && packet.rolloverCount > 0)
  ...
  const lines = [
    ...
    "",
    "### Current Packet",
    ...formatPacketBullets(nowPackets, (packet) => [
      packet.goal || "Goal not set yet",
      packet.targetFinish ? `target finish ${packet.targetFinish}` : "",
    ...
    "",
    "### Standby Packet",
    ...formatPacketBullets(standbyPackets, (packet) => [
      packet.whyNow || "Why-now note missing",
      packet.targetStart ? `target start ${packet.targetStart}` : "",
      packet.localProjectIds.length === 1
        ? resolveProjectTitle(input.projects, packet.localProjectIds[0] ?? "")
    ...
    "",
    "### Open Decisions",
    ...formatDecisionBullets(openDecisions, (decision) => [
      decision.decisionType ? `type ${decision.decisionType.toLowerCase()}` : "",
      decision.revisitBy ? `revisit ${decision.revisitBy}` : "",
      decision.localProjectIds.length === 1
        ? resolveProjectTitle(input.projects, decision.localProjectIds[0] ?? "")
        : `${decision.localProjectIds.length} linked projects`,
    ], "- No open decisions."),
    "",
    "### Blocked Tasks",
    ...formatTaskBullets(blockedTasks, (task) => [
      task.dueDate ? `due ${task.dueDate}` : "",
      task.priority ? `priority ${task.priority}` : "",
      task.workPacketIds.length === 1 ? `packet linked` : "packet link missing",
    ], "- No blocked tasks."),
    "",
    "### Overdue Tasks",
    ...formatTaskBullets(overdueTasks, (task) => [
      task.dueDate ? `due ${task.dueDate}` : "",
      task.status ? `status ${task.status.toLowerCase()}` : "",
      task.priority ? `priority ${task.priority}` : "",
    ], "- No overdue tasks."),
    "",
    "### Rollover Risk",
    ...formatPacketBullets(rolloverRisk, (packet) => [
...
export function renderWeeklyExecutionSection(input: {
  ...
  const lines = [
    ...
    "",
    "### Current Packet",
    ...formatPacketBullets(input.nowPackets, (packet) => [
      packet.goal || "Goal not set yet",
      packet.targetFinish ? `target finish ${packet.targetFinish}` : "",
      packet.status ? `status ${packet.status.toLowerCase()}` : "",
    ...
    "",
    "### Standby Packet",
    ...formatPacketBullets(input.standbyPackets, (packet) => [
      packet.whyNow || "Why-now note missing",
      packet.targetStart ? `target start ${packet.targetStart}` : "",
      packet.status ? `status ${packet.status.toLowerCase()}` : "",
    ...
    "",
    "### Decisions Made",
    ...formatDecisionBullets(input.decisionsCommitted, (decision) => [
      decision.decisionType ? `type ${decision.decisionType.toLowerCase()}` : "",
    ...
    "",
    "### Blocked Work",
    ...formatTaskBullets(input.blockedTasks, (task) => [
      task.priority ? `priority ${task.priority}` : "",
      task.dueDate ? `due ${task.dueDate}` : "",
      task.taskNotes || "",
    ...
    "",
    "### Completed Task Flow",
    ...formatTaskBullets(input.completedTasks, (task) => [
    ...
    "",
    "### Rollover Packets",
    ...formatPacketBullets(input.rolloverPackets, (packet) => [
      `rollovers ${packet.rolloverCount}`,
      packet.targetFinish ? `target finish ${packet.targetFinish}` : "",
      packet.whyNow || "",
...
function parseStrategy(raw: unknown): LocalPortfolioExecutionViewPlan["strategy"] {
  if (!raw || typeof raw !== "object") {
    throw new AppError("Local portfolio execution views config is missing strategy");
  }

  const strategy = raw as Record<string, unknown>;
  if (strategy.primary !== "notion_mcp") {
    throw new AppError('Local portfolio execution views strategy.primary must be "notion_mcp"');
  }
  if (strategy.fallback !== "playwright") {
    throw new AppError('Local portfolio execution views strategy.fallback must be "playwright"');
  }
  if (!Array.isArray(strategy.notes) || strategy.notes.some((entry) => typeof entry !== "string")) {
    throw new AppError("Local portfolio execution views strategy.notes must be a string array");
  }
...
function parseCollections(raw: unknown): LocalPortfolioExecutionViewCollection[] {
  if (!Array.isArray(raw) || raw.length === 0) {
    throw new AppError("Local portfolio execution views config must include collections");
  }
  ...
  return raw.map((entry, index) => {
    ...
    if (key !== "decisions" && key !== "packets" && key !== "tasks") {
      throw new AppError(`collections[${index}].key must be decisions, packets, or tasks`);
    }
    const database = parseExecutionDataSource(value.database, `collections[${index}].database`);
    const views = parseExecutionViews(value.views, `collections[${index}].views`);
    return {
      key,
      database,
      views,
...
function parseExecutionViews(raw: unknown, fieldName: string): LocalPortfolioExecutionViewSpec[] {
  if (!Array.isArray(raw) || raw.length === 0) {
...
export function parseExecutionDataSource(raw: unknown, fieldName: string): ExecutionDataSourceRef {
  if (!raw || typeof raw !== "object") {
...
function resolveProjectTitle(projects: ControlTowerProjectRecord[], projectId: string): string {
...
function formatBuildSessionBullets(
  ...
  return sessions.map((session) => {
    const details = [session.sessionDate ? `date ${session.sessionDate}` : "", session.outcome || ""]
      .filter(Boolean)
      .join(" | ");
