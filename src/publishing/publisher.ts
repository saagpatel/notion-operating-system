...
export class Publisher {
	...
	public async publish(
		...
		if (resolvedDestination.destinationType === "page") {
			const built = buildPageDestinationProperties(
				destination,
				parsed,
	...
	private async publishToPageParent({
		...
		if (destination.mode !== "create_new_page") {
			throw new AppError(
				`Page destinations only support create_new_page in this project`,
			);
		}
	...
	private async publishToStandalonePage({
		...
		if (destination.mode === "create_new_page") {
			throw new AppError(
				"Standalone page updates do not support create_new_page",
			);
		}
		...
		let expectation: WriteExpectation = { kind: "contains", updates: [] };
		...
		if (destination.mode === "replace_full_content") {
			...
			const contentUpdates = validateContentUpdates(request.contentUpdates);
			await liveApi.patchPageMarkdown({
				pageId: targetPage.id,
			...
			expectation = { kind: "contains", updates: contentUpdates };
		} else if (
			destination.mode === "update_existing_page" &&
			request.contentUpdates?.length
		) {
			await liveApi.patchPageMarkdown({
				pageId: targetPage.id,
				command: "update_content",
			...
			expectation = { kind: "contains", updates: request.contentUpdates };
		}
	...
	private async publishToDataSource({
		...
		if (destination.mode === "create_new_page") {
			const template = api
				? await resolveTemplate(
						api,
						destination,
						resolvedDestination.dataSourceId,
					)
				: undefined;
			...
			const wroteFullContentLocally =
				!template ||
				(parsedBody.trim().length > 0 &&
					destination.postTemplatePatchMode !== "none");

			if (wroteFullContentLocally && template) {
			...
			const expectation: WriteExpectation = wroteFullContentLocally
				? { kind: "full", markdown: parsedBody }
				: { kind: "none", reason: "template content is not locally known" };
		...
		if (!targetPage) {
			throw new AppError(
				`Could not locate a target page for alias "${destination.alias}"`,
			);
		}
	...
	/**
	 * Compares a Publisher readback against what we meant to write and applies
	 * the destination's `verifyWrites` knob (default "warn" when unset).
	 * "fail" throws only on genuine divergence; "unverifiable" never throws,
	 * it only warns — a template we cannot inspect locally is an honest gap,
	 * not a failed write.
	 */
	private async verifyWrite({
		...
		const mode: VerifyWritesMode =
			destination.safeDefaults.verifyWrites ?? "warn";

		if (mode === "off") {
			return { status: "unverifiable", reason: "verification disabled" };
		}
		...
		await this.logger.warn("write_verification_unverifiable", {
			destinationAlias: destination.alias,
			reason: result.reason,
		});
		warnings.push(`Write verification unavailable: ${result.reason}`);
		return result;
	}
	...
	private requireApi(): NotionApi {
		if (!this.api) {
			throw new AppError(
				"NOTION_TOKEN is required for live validation or live publishing",
			);
		}
...
async function resolveTemplate(
	...
	if (destination.templateMode === "default") {
		...
		if (!template) {
			throw new AppError(
				`No default template found for alias "${destination.alias}"`,
			);
		}
		return template;
	}
	...
	if (destination.templateId) {
		const template = templates.find(
			(entry) => entry.id === destination.templateId,
		);
		if (!template) {
			throw new AppError(
				`Template ${destination.templateId} not found for alias "${destination.alias}"`,
			);
		}
		return template;
	}
	...
	if (destination.templateName) {
		const template = templates.find(
			(entry) => entry.name === destination.templateName,
		);
		if (!template) {
			throw new AppError(
				`Template "${destination.templateName}" not found for alias "${destination.alias}"`,
			);
		}
		return template;
	}
	...
	throw new AppError(
		`Destination "${destination.alias}" uses templateMode=specific but has no templateId or templateName`,
	);
}
...
async function resolveTargetPage({
	...
	if (!lookup) {
		throw new AppError(
			`Destination "${destination.alias}" requires lookup rules for update modes`,
		);
	}
	...
	if (lookup.by === "page_id") {
		if (!lookup.value) {
			throw new AppError(
				`Lookup rule "page_id" for "${destination.alias}" requires a value`,
			);
		}
		if (dryRun && !api) {
			return {
	...
	if (lookup.by === "url") {
		if (!lookup.value) {
			throw new AppError(
				`Lookup rule "url" for "${destination.alias}" requires a value`,
			);
		}
		if (dryRun && !api) {
			return {
	...
	const effectiveTitle =
		lookup.by === "title"
			? lookup.value || resolveLookupTitle(destination, title)
			: undefined;
...
function collectReadWarnings(
	...
	if (readback.unknownBlockIds.length > 0) {
		warnings.push(
			`Final markdown readback contained unknown_block_ids: ${readback.unknownBlockIds.join(", ")}`,
		);
	}
}
