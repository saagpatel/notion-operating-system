...
The real checkout is `~/Projects/Notion`. The historical `~/Notion` path is a hollow stub containing only a `.git/hooks/reference-transaction` tripwire hook; `git status` there fails with `fatal: not a git repository`. Do not write into `~/Notion`.

The Fable integration is merged into `main`; verify `origin/main` live before relying on publication status. The source branches are `docs/fable-explore-session` and `codex/fable-integration-closeout-20260711`. Always check `git status --short --branch`, and do not treat this file as the source of truth for the current branch or working tree.
...
- Fable/public material, build-log idempotency, config eviction, signal watermarks, and publisher write verification are merged into `main`; verify the remote before relying on push status.
...
- Latest local portfolio truth lives at `~/Projects/GithubRepoAuditor/output/portfolio-truth-latest.json`. It regenerates frequently, so do not copy counts or active-project sets from this handoff. Re-query the JSON before using portfolio counts in another doc:
  `jq '{generated_at,total:(.projects|length),counts:.source_summary.attention_state_counts}' ~/Projects/GithubRepoAuditor/output/portfolio-truth-latest.json`.
...
3. The remaining orphan candidates should not be promoted automatically: most are manual-only, `evals` and `hermes-harness-foundation` are experiments, and `GithubRepoAuditor-public` is unsafe title drift against the real `~/Projects/GithubRepoAuditor` repo.
