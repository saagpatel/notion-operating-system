...
describe("local portfolio execution system", () => {
	...
	test("renders execution artifacts with durable managed sections", () => {
		...
		const weekly = renderWeeklyExecutionSection({
			...
			standbyPackets: [],
			decisionsCommitted: [
				baseDecision({ status: "Committed", decidedOn: TODAY }),
			],
			blockedTasks: [baseTask({ status: "Blocked" })],
			completedTasks: [baseTask({ status: "Done", completedOn: TODAY })],
			rolloverPackets: [
				basePacket({ rolloverCount: 1, title: "Rollover packet" }),
			],
...
function baseProject(
	...
	return {
		...
		oneLinePitch:
			overrides.oneLinePitch ?? "A phase-two execution test project",
...
function baseDecision(
	overrides: Partial<ProjectDecisionRecord> = {},
	...
	return {
		...
		optionsConsidered:
			overrides.optionsConsidered ?? "Resume, pause, or narrow scope",
...
function basePacket(
	...
	return {
		...
		whyNow:
			overrides.whyNow ?? "This is the best low-friction project to resume.",
