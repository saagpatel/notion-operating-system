...
describe("cohort rollout", () => {
  test("defaults to the fixed four-project cohort in deterministic order", () => {
    expect(parseCohortProjectSelection()).toEqual(["BattleGrid", "EarthPulse", "Relay", "SynthWave"]);
    expect(parseCohortProjectSelection("Relay,BattleGrid,SynthWave,EarthPulse")).toEqual([
      "BattleGrid",
      "EarthPulse",
      "Relay",
      "SynthWave",
  ...
  test("builds the four-project plan and excludes EarthPulse-readiness", () => {
    const plan = buildCohortRolloutPlan({
      selectedTitles: parseCohortProjectSelection("BattleGrid,EarthPulse,Relay,SynthWave"),
      projects: [
        baseProject({ id: "battlegrid", title: "BattleGrid" }),
        baseProject({ id: "earthpulse", title: "EarthPulse" }),
        baseProject({ id: "earthpulse-readiness", title: "EarthPulse-readiness" }),
    ...
    expect(plan.orderedTitles).toEqual(["BattleGrid", "EarthPulse", "Relay", "SynthWave"]);
    expect(plan.projects).toHaveLength(4);
    expect(plan.projects.map((project) => project.title)).toEqual(["BattleGrid", "EarthPulse", "Relay", "SynthWave"]);
    ...
    expect(plan.projects.every((project) => project.classification === "move to GitHub next")).toBe(true);
    expect(plan.projects.every((project) => project.githubLane === "active_allowlisted")).toBe(true);
    expect(plan.projects.find((project) => project.title === "SynthWave")?.sourceAction).toBe("upsert_active");
  });

  test("rewrites next moves into issue-focused rollout language", () => {
    expect(buildCohortIssueNextMove("Run cargo tauri dev and capture the first blocker.")).toContain(
      "create and work from the governed GitHub issue",
    );
  });
});
...
function baseProject(overrides: Partial<ControlTowerProjectRecord> = {}): ControlTowerProjectRecord {
  return {
    ...
    momentum: overrides.momentum ?? "Hot",
    needsReview: overrides.needsReview ?? true,
    nextMove: overrides.nextMove ?? "Run the main dev command and capture the first blocker.",
    biggestBlocker: overrides.biggestBlocker ?? "",
    lastActive: overrides.lastActive ?? "2026-03-17",
    dateUpdated: overrides.dateUpdated ?? "2026-03-17",
    ...
    operatingQueue: overrides.operatingQueue ?? ("Needs Review" as OperatingQueue),
