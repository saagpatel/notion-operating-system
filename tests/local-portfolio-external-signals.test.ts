...
describe("local portfolio external signals", () => {
	test("parses the phase-5 source, provider, and view configs", async () => {
		...
		const sourceConfig =
			parseLocalPortfolioExternalSignalSourceConfig(sourcesRaw);
	...
	test("builds bounded seed plans for the priority telemetry slice", async () => {
		...
		const plans = buildExternalSignalSeedPlans({
			...
			packets: [
				basePacket({ localProjectIds: ["project-2"], priority: "Now" }),
			],
			sourceConfig,
		});
		...
		expect(
			plans.some(
				(plan) =>
					plan.localProjectId === "project-1" && plan.provider === "GitHub",
			),
		).toBe(true);
		expect(
			plans.some(
				(plan) =>
					plan.localProjectId === "project-2" && plan.provider === "Vercel",
			),
		).toBe(true);
	...
	test("derives external summaries and additive recommendation boosts", () => {
		...
		const adjustments = buildExternalRecommendationAdjustments(
			project,
			summary,
		);
				...
				projects: {
					...
					properties: {
						...
						"External Signal Coverage": {
							name: "External Signal Coverage",
							type: "select",
							writable: true,
						},
						"Latest External Activity": {
							name: "Latest External Activity",
							type: "date",
							writable: true,
						},
						"External Signal Updated": {
							name: "External Signal Updated",
							type: "date",
							writable: true,
						},
						...
						"Recent Failed Workflow Runs": {
							name: "Recent Failed Workflow Runs",
							type: "number",
							writable: true,
						},
						"Recommendation Lane": {
							name: "Recommendation Lane",
							type: "select",
							writable: true,
						},
					},
	...
	test("renders recent signal bullets in a stable order", () => {
		...
		expect(markdown.indexOf("[B Event]")).toBeLessThan(
			markdown.indexOf("[A Event]"),
		);
	});
...
function baseSyncRun(
	overrides: Partial<ExternalSignalSyncRunRecord> = {},
