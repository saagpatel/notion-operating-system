...
const SCHEMA: DataSourceSchemaSnapshot = {
  ...
  properties: {
    ...
    "Portfolio Call": { name: "Portfolio Call", type: "select", writable: true },
    "One-Line Pitch": { name: "One-Line Pitch", type: "rich_text", writable: true },
    "Next Move": { name: "Next Move", type: "rich_text", writable: true },
    "Biggest Blocker": { name: "Biggest Blocker", type: "rich_text", writable: true },
    Momentum: { name: "Momentum", type: "select", writable: true },
    "Last Active": { name: "Last Active", type: "date", writable: true },
    "Start Here": { name: "Start Here", type: "rich_text", writable: true },
    "Primary Run Command": { name: "Primary Run Command", type: "rich_text", writable: true },
    "Setup Friction": { name: "Setup Friction", type: "select", writable: true },
    "Runs Locally": { name: "Runs Locally", type: "select", writable: true },
    "Last Meaningful Work": { name: "Last Meaningful Work", type: "rich_text", writable: true },
    "Build Maturity": { name: "Build Maturity", type: "select", writable: true },
    "Ship Readiness": { name: "Ship Readiness", type: "select", writable: true },
    "Effort to Demo": { name: "Effort to Demo", type: "select", writable: true },
    "Effort to Ship": { name: "Effort to Ship", type: "select", writable: true },
    "Value / Outcome": { name: "Value / Outcome", type: "rich_text", writable: true },
    "Monetization / Strategic Value": {
      name: "Monetization / Strategic Value",
      type: "rich_text",
      writable: true,
    },
    "Needs Review": { name: "Needs Review", type: "checkbox", writable: true },
    "Evidence Confidence": { name: "Evidence Confidence", type: "select", writable: true },
...
describe("local portfolio view validation", () => {
  test("validates the repo view config against the expected schema", async () => {
    ...
    expect(summary.validatedViews.every((view) => Boolean(view.viewId))).toBe(true);
  });
  ...
  test("fails when a referenced property is missing", async () => {
    ...
    expect(() => validateLocalPortfolioViewPlanAgainstSchema(plan, SCHEMA)).toThrow(
