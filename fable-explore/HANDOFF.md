...
3. **Finish the remaining explainer QA.** The 2026-07-11 browser pass served the ES module over HTTP, exercised the Act 1 lost-ack path, opened all six acts, found no console errors, and found no horizontal overflow at a real 390×844 browser viewport. Still outstanding: (a) exhaustive predict-then-reveal click-through of every beat in Act 3 with all three failure toggles; and (b) a runtime-emulated reduced-motion check (`prefers-reduced-motion` → packets teleport with fade). The reduced-motion CSS contract was inspected, but the available browser session did not emulate the preference. Keep using a real browser viewport/device emulation rather than Chrome's `--window-size` alone.
