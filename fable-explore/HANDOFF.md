...
5. **Resolved — the four-diagram set is built locally.** `fable-explore/diagrams/` contains standalone SVGs for the protocol taxonomy, five-rung write ladder, receipt loop/crash-window repair, and weekly preflight→live gate. All four use the explainer ledger palette, direct labels, accessible titles/descriptions, and dependency-free `1200×760` viewBoxes. XML validation and browser render QA passed after correcting one clipped receipt-loop label. These are source assets only; site integration and publication remain separately gated.
6. **Diagram site integration is staged locally; publication remains gated.** Live re-grounding found that the six-act explainer was already integrated and published at `/authority` on the `portfolio-index` production line, so no duplicate explainer page or essay registration was created. The new diagram gallery is isolated at `/Users/d/Projects/_codex-worktrees/fable-explainer-site-staging` on branch `codex/fable-explainer-site-staging`, commit `cafd0fd`. It adds the four standalone SVGs beneath the existing interactive and refreshes the affected corpus records. The full 25-stage site gate passed, including 1440/390/320 visual shards, and the post-commit idempotency check matched committed bytes. The branch is local and unpushed; no deploy or production mutation occurred. Shipping still requires a separate operator decision and the canonical production merge/deploy/readback sequence.
